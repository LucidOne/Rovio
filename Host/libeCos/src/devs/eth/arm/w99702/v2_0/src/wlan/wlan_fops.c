 ...
 */
int
fw_read(char *name, cyg_uint8 ** addr, cyg_uint32 * len)
{
    struct file *fp;
    int ret;
    cyg_uint8 *ptr;
    ...
    if (fp == NULL) {
        diag_printf("Could not open file:%s\n", name);
    ...
    *len = fp->f_dentry->d_inode->i_size - fp->f_pos;
    ...
    diag_printf("len=%x\n", *len);
    ptr = (cyg_uint8 *) vmalloc(*len + 1023);
    if (ptr == NULL) {
        diag_printf("vmalloc failure\n");
        return WLAN_STATUS_FAILURE;
    }
    if (wlan_fread(fp, ptr, *len) > 0) {
        *addr = ptr;
        diag_printf("success to read the file %s \n", name);
        ret = WLAN_STATUS_SUCCESS;
    } else {
        fw_buffer_free(ptr);
        *addr = NULL;
        diag_printf("fail to read the file %s \n", name);
        ret = WLAN_STATUS_FAILURE;
    }

    wlan_fclose(fp);
    diag_printf("*addr=%p ptr=%p len %x\n", *addr, ptr, *len);
    return ret;
}
