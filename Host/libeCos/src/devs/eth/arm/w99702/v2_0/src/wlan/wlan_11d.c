...
extern CHANNEL_FREQ_POWER *wlan_get_region_cfp_table(cyg_uint8 region,
                                                     cyg_uint8 band, int *cfp_no);
...
*/
static cyg_uint8
wlan_region_2_code(cyg_int8 * region)
{
    cyg_uint8 i;
    cyg_uint8 size = sizeof(region_code_mapping) / sizeof(region_code_mapping_t);
...
*/
static cyg_uint8 *
wlan_code_2_region(cyg_uint8 code)
{
    cyg_uint8 i;
    cyg_uint8 size = sizeof(region_code_mapping) / sizeof(region_code_mapping_t);
    for (i = 0; i < size; i++) {
        if (region_code_mapping[i].code == code)
            return (cyg_uint8*)(region_code_mapping[i].region);
    }
    /* default is US */
    return (cyg_uint8*)(region_code_mapping[0].region);
}
...
*/
static BOOLEAN
wlan_get_chan_11d(cyg_uint8 band, cyg_uint8 firstChan, cyg_uint8 NoOfChan, cyg_uint8 * chan)
...
*/
BOOLEAN
wlan_channel_known_11d(cyg_uint8 chan, parsed_region_chan_11d_t * parsed_region_chan)
{
    chan_power_11d_t *chanPwr = parsed_region_chan->chanPwr;
    cyg_uint8 NoOfChan = parsed_region_chan->NoOfChan;
    cyg_uint8 i = 0;
    ...
    for (i = 0; i < NoOfChan; i++) {
        if (chan == chanPwr[i].chan) {
            diag_printf("11D: Found Chan:%d\n", chan);
            LEAVE();
            return TRUE;
        }
    }

    diag_printf("11D: Not Find Chan:%d\n", chan);
    LEAVE();
    return FALSE;
}
...
*/
cyg_uint32
chan_2_freq(cyg_uint8 chan, cyg_uint8 band)
{
...
{
    cyg_uint8 NoOfSubband = 0;

    cyg_uint8 NoOfChan = parsed_region_chan->NoOfChan;
    cyg_uint8 NoOfParsedChan = 0;

    cyg_uint8 firstChan = 0, nextChan = 0, maxPwr = 0;

    cyg_uint8 i, flag = 0;

    ENTER();

    memcpy(domaininfo->CountryCode, (void *)parsed_region_chan->CountryCode,
           COUNTRY_CODE_LEN);

    diag_printf("11D:NoOfChan=%d\n", NoOfChan);
                                     ...
                                     parsed_region_chan)
{
    ...
    if (region_chan == NULL) {
        diag_printf("11D: region_chan is NULL\n");
        return;
    }

    cfp = region_chan->CFP;
    if (cfp == NULL) {
        diag_printf("11D: cfp equal NULL \n");
        return;
    }

    parsed_region_chan->band = region_chan->Band;
    parsed_region_chan->region = region_chan->Region;
    memcpy((void *)parsed_region_chan->CountryCode,
           wlan_code_2_region(region_chan->Region), COUNTRY_CODE_LEN);

    diag_printf("11D: region[0x%x] band[%d]\n", parsed_region_chan->region,
           parsed_region_chan->band);

    for (i = 0; i < region_chan->NrCFP; i++, cfp++) {
        parsed_region_chan->chanPwr[i].chan = cfp->Channel;
        parsed_region_chan->chanPwr[i].pwr = cfp->MaxTxPower;
        diag_printf("11D: Chan[%d] Pwr[%d]\n",
...
*/
BOOLEAN
wlan_region_chan_supported_11d(cyg_uint8 region, cyg_uint8 band, cyg_uint8 chan)
{
    CHANNEL_FREQ_POWER *cfp;
    int cfp_no;
    cyg_uint8 idx;
...
{
    cyg_uint8 NoOfSubband, NoOfChan;
    cyg_uint8 lastChan, firstChan, curChan;
    cyg_uint8 region;

    cyg_uint8 idx = 0;                 /*chan index in parsed_region_chan */
    ...
    HEXDUMP("CountryInfo:", (cyg_int8 *) CountryInfo, 30);
    ...
    parsed_region_chan->region = region =
        wlan_region_2_code((void *)CountryInfo->CountryCode);

    diag_printf("regioncode=%x\n", (cyg_uint8) parsed_region_chan->region);
    HEXDUMP("CountryCode:", (char *) CountryInfo->CountryCode,
            COUNTRY_CODE_LEN);

    parsed_region_chan->band = band;

    memcpy((void *)parsed_region_chan->CountryCode, (void *)CountryInfo->CountryCode,
           COUNTRY_CODE_LEN);
    ...
    for (j = 0, lastChan = 0; j < NoOfSubband; j++) {
        ...
        if (CountryInfo->Subband[j].FirstChan <= lastChan) {
            ...
            diag_printf("11D: Chan[%d>%d] Overlap\n",
        ...
        for (i = 0; idx < MAX_NO_OF_CHAN && i < NoOfChan; i++) {
            ...
            if (wlan_get_chan_11d(band, firstChan, i, &curChan)
                ...
                diag_printf("chan is not supported: %d \n", i);
                break;
            }
    ...
    diag_printf("NoOfChan=%x\n", parsed_region_chan->NoOfChan);
    HEXDUMP("11D:parsed_region_chan:", (cyg_int8 *) parsed_region_chan,
...
*/
cyg_uint8
wlan_get_scan_type_11d(cyg_uint8 chan, parsed_region_chan_11d_t * parsed_region_chan)
{
    cyg_uint8 scan_type = HostCmd_SCAN_TYPE_PASSIVE;

    ENTER();

    if (wlan_channel_known_11d(chan, parsed_region_chan)) {
        diag_printf("11D: Found and do Active Scan\n");
        scan_type = HostCmd_SCAN_TYPE_ACTIVE;
    } else {
        diag_printf("11D: Not Find and do Passive Scan\n");
    }
...
wlan_init_11d(wlan_private * priv)
{
    ...
    memset((void *)&(priv->adapter->parsed_region_chan), 0,
           sizeof(parsed_region_chan_11d_t));
...
wlan_enable_11d(wlan_private * priv, state_11d_t flag)
{
    ...
    if (ret) {
        diag_printf("11D: Fail to enable 11D \n");
    }
...
wlan_set_domain_info_11d(wlan_private * priv)
{
    ...
    if (!wlan_get_state_11d(priv)) {
        diag_printf("11D: dnld domain Info with 11d disabled\n");
        return WLAN_STATUS_SUCCESS;
    }
    ...
    if (ret) {
        diag_printf("11D: Fail to dnld domain Info\n");
    }
...
*/
int
wlan_set_universaltable(wlan_private * priv, cyg_uint8 band)
{
    wlan_adapter *Adapter = priv->adapter;
    cyg_uint16 size = sizeof(CHANNEL_FREQ_POWER);
    cyg_uint16 i = 0;
...

    {
        ...
        diag_printf("11D: BG-band NrCFP=%d\n",
               Adapter->universal_channel[i].NrCFP);
                             ...
                             cyg_uint16 CmdOption)
{
    ...
    wlan_adapter *Adapter = priv->adapter;
    cyg_uint8 NoOfSubband = Adapter->DomainReg.NoOfSubband;
    ...
    if (CmdOption == HostCmd_ACT_GET) {
        ...
        HEXDUMP("11D: 802_11D_DOMAIN_INFO:", (cyg_uint8 *) cmd, (int) (cmd->Size));
        LEAVE();
        return WLAN_STATUS_SUCCESS;
    }

    domain->Header.Type = wlan_cpu_to_le16(TLV_TYPE_DOMAIN);
    memcpy((void *)domain->CountryCode, Adapter->DomainReg.CountryCode,
    ...
    if (NoOfSubband) {
        memcpy((void *)domain->Subband, (void *)Adapter->DomainReg.Subband,
...
wlan_ret_802_11d_domain_info(wlan_private * priv, HostCmd_DS_COMMAND * resp)
{
    ...
    cyg_uint16 Action = wlan_le16_to_cpu(domaininfo->Action);
    cyg_int16 ret = WLAN_STATUS_SUCCESS;
    cyg_uint8 NoOfSubband = 0;

    ENTER();

    HEXDUMP("11D DOMAIN Info Rsp Data:", (cyg_uint8 *) resp,
            (int) wlan_le16_to_cpu(resp->Size));

    NoOfSubband = (domain->Header.Len - 3) / sizeof(IEEEtypes_SubbandSet_t);
    /* countrycode 3 bytes */

    diag_printf("11D Domain Info Resp: NoOfSubband=%d\n", NoOfSubband);

    if (NoOfSubband > MRVDRV_MAX_SUBBAND_802_11D) {
        diag_printf("Invalid Numrer of Subband returned!!\n");
...
wlan_parse_dnld_countryinfo_11d(wlan_private * priv)
{
    ...
    if (wlan_get_state_11d(priv) == ENABLE_11D) {
        ...
        memset((void *)&Adapter->parsed_region_chan, 0,
               sizeof(parsed_region_chan_11d_t));
        ...
        if (ret == WLAN_STATUS_FAILURE) {
            diag_printf("11D: Err Parse domain_info from AP..\n");
            LEAVE();
            return ret;
        }
        ...
        if (ret) {
            diag_printf("11D: Err set domainInfo to FW\n");
            LEAVE();
            return ret;
        }
    }
...
wlan_create_dnld_countryinfo_11d(wlan_private * priv)
{
    ...
    ENTER();
    diag_printf("11D:CurBssParams.Band[%d]\n", Adapter->CurBssParams.band);
    ...
    if (wlan_get_state_11d(priv) == ENABLE_11D) {
        ...
        for (j = 0; j < sizeof(Adapter->region_channel) /
            ...
            diag_printf("11D:[%d] region_chan->Band[%d]\n", j,
                   region_chan->Band);
        ...
        if (j >= sizeof(Adapter->region_channel) /
            ...
            diag_printf("11D:region_chan not found. Band[%d]\n",
                   Adapter->CurBssParams.band);
            LEAVE();
            return WLAN_STATUS_FAILURE;
        }
