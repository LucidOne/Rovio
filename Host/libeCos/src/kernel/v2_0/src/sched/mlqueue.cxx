...
volatile cyg_ucount32 *cyg_scheduler_sched_lock = &(Cyg_Scheduler_SchedLock::sched_lock);
...
Cyg_Scheduler_Implementation::schedule(void)
{
    ...
    Cyg_RunQueue *queue = &run_queue[index];
//diag_printf("start cpu index=%x schedule=%x,&queue_map=%x,queue_map=%x\n",index,run_queue,&queue_map,queue_map);
//diag_printf("queue=%x\n",queue);
//diag_printf("******queue_map=%x\n",queue_map);
    CYG_ASSERT( !queue->empty(), "Queue for index empty");
//diag_printf("get_head\n");
    Cyg_Thread *thread = queue->get_head();
//diag_printf("thread=%x\n",thread);
#endif
...
//diag_printf("add_thread run_queue=%x,pri=%d,&queue_map=%x,queue_map=%x\n",run_queue,pri,&queue_map,queue_map);
...
//diag_printf("set_idle_thread call resume\n");
    thread->resume();
...
#else    
//diag_printf("timeslice_count=%d\n",timeslice_count[CYG_KERNEL_CPU_THIS()]);
