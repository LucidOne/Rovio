...
externC cyg_uint64 cyg_current_time(void);
...
//diag_printf("01 current=%x,state=%x\n",current,current->state);
//diag_printf("02 get_need_reschedule:%d,%x\n",get_need_reschedule(),need_reschedule);
...
if(0)//current != next )
{
	diag_printf("current=%x,next=%x,state=%d, time=%d\n",current,next,current->state, cyg_current_time() - lastTime);
	lastTime = cyg_current_time();
}
            if( current != next )
            {
...
//diag_printf("04 &current_thread[0]:%x,next=%x,cu->stack=%x,next->stack=%x\n",&current_thread[0],next,current->stack_ptr,next->stack_ptr);
                // Switch contexts
                HAL_THREAD_SWITCH_CONTEXT( &current->stack_ptr,
                                           &next->stack_ptr );
//diag_printf("05 sched.cxx return current=%x\n",current);
