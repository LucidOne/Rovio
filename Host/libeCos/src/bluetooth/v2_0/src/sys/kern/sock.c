...
extern int selrwwakeup(struct sock *sk);

/* Run time adjustable parameters. */
cyg_uint32 sysctl_wmem_max = SK_WMEM_MAX;
cyg_uint32 sysctl_rmem_max = SK_RMEM_MAX;
cyg_uint32 sysctl_wmem_default = SK_WMEM_MAX;
cyg_uint32 sysctl_rmem_default = SK_RMEM_MAX;
		    ...
		    char *optval, int *optlen)
{
	...
	if(len < 0)
  	...
  	switch(optname) 
  	{
		...
		case SO_DONTROUTE:
			v.val = (int)sk->localroute;
			break;
		
		case SO_BROADCAST:
			v.val= (int)sk->broadcast;
			break;
...
//		case SO_ERROR:
//			v.val = (int)-sock_error(sk);
//			if(v.val==0)
//				v.val=(int)xchg(&sk->err_soft,0);
//			break;

		case SO_OOBINLINE:
			v.val = (int)sk->urginline;
			break;
	
		case SO_NO_CHECK:
			v.val = (int)sk->no_check;
			break;

		case SO_PRIORITY:
			v.val = (int)sk->priority;
			break;
...
{
	struct sock *sk = malloc(sizeof(*sk));//kmem_cache_alloc(sk_cachep, priority);

//printf("sk_alloc=%x %x;",sk,&sk->socket);
	if(sk && zero_it) {
...
void sk_free(struct sock *sk)
{
	...
	if (atomic_read(&sk->omem_alloc))
		diag_printf("sk_free: optmem leakage (%d bytes) detected.\n", atomic_read(&sk->omem_alloc));
...
//printf("sock_wfree=%x ",__builtin_return_address(0));
		...
		if (atomic_read(&sk->wmem_alloc) < sk->sndbuf) {
			...
			if (skb) {
				...
				for (i = 0; i < npages; i++) {
					...
					page = malloc(PAGE_SIZE);//alloc_pages(sk->allocation, 0);
					if (!page) {
						err = ENOBUFS;
						kfree_skb(skb);
						goto failure;
...
void sklist_destroy_socket(struct sock **list,struct sock *sk)
{
	...
	else
	{
		...
		//add_timer(&sk->timer);
		cyg_alarm_initialize(sk->timeout_alarm_handle, cyg_current_time()+SOCK_DESTROY_TIME, 0);
		cyg_clock_to_counter(cyg_real_time_clock(), &sk->h);
    	cyg_alarm_create(sk->h, (cyg_alarm_t *)sklist_destroy_timer, (cyg_addrword_t)sk, &sk->timeout_alarm_handle, &sk->timeout_alarm);
...
void show_threads()
{
	...
	while(cyg_thread_get_next(&thread, &id)){
		...
		if(!cyg_thread_get_info(thread,id,&info))
			break;
		printf("name=%s,state=%d",info.name?info.name:"----",info.state);
	}
}
...
//	printf("wakeup sk=%x,&sk->sem=%x,state=%d",sk,&sk->sem,sk->state);
	cyg_semaphore_post(&sk->sem);//wake_up_interruptible_all(sk->sleep);
	selrwwakeup(sk);
//	printf("&sk->sem.count=%x,count=%d",&sk->sem.count,sk->sem.count);
...
void sock_def_error_report(struct sock *sk)
{
	//read_lock(&sk->callback_lock);
	//if (sk->sleep && waitqueue_active(sk->sleep))
//	printf("sock_def_error_report=%x ",__builtin_return_address(0));
		cyg_semaphore_post(&sk->sem);//wake_up_interruptible(sk->sleep);
		selrwwakeup(sk);
...
void sock_def_readable(struct sock *sk, int len)
{
	...
	//if (sk->sleep && waitqueue_active(sk->sleep))
	//diag_printf("sk = %x\n",sk);
//	printf("sock_def_readable=%x ",__builtin_return_address(0));
	selrwwakeup(sk);
	cyg_semaphore_post(&sk->sem);//wake_up_interruptible(sk->sleep);
...
{
	read_lock(&sk->callback_lock);
//printf("sock_def_write_space=%x ",__builtin_return_address(0));
...
void sock_def_destruct(struct sock *sk)
{
	if (sk->protinfo.destruct_hook)
		free(sk->protinfo.destruct_hook);
}
...
//	BT_DBG("sk->socket %p sk->refcnt=%d",sk->socket,sk->refcnt);
}
#endif
