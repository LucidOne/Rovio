			...
			int len)
{
	...
	while (len > 0)
	{
		cyg_uint8 *base = (cyg_uint8*)iov->iov_base + offset;
		int copy = min_t(unsigned int, len, (char*)iov->iov_len - offset);
				 ...
				 int offset, unsigned int len, int *csump)
{
	...
	while (len > 0)
	{
		...
		if (copy) {
			csum = csum_and_copy_from_user((const char*)base, (char*)kdata, copy,
							csum, &err);
			if (err)
				goto out;
		}
