...
static int l2cap_send_req(struct l2cap_conn *conn, cyg_uint8 code, cyg_uint16 len, void *data);
static int l2cap_send_rsp(struct l2cap_conn *conn, cyg_uint8 ident, cyg_uint8 code, cyg_uint16 len, void *data);

/* ----- L2CAP timers ------ */
static void l2cap_sock_timeout(cyg_addrword_t arg)
{
...
//	sk->timer.data = (unsigned long)sk;
	...
	//cyg_semaphore_init(&bp->sem, 0);
	
    cyg_clock_to_counter(cyg_real_time_clock(), &sk->h);
    cyg_alarm_create(sk->h, (cyg_alarm_t*)l2cap_sock_timeout, (cyg_addrword_t)sk, &sk->timeout_alarm_handle, &sk->timeout_alarm);
	
}

/* -------- L2CAP connections --------- */
static struct l2cap_conn *l2cap_conn_add(struct hci_conn *hcon, cyg_uint8 status)
{
	...
	if (!(conn = malloc(sizeof(struct l2cap_conn))))
		return NULL;
	...
	cyg_spinlock_init(&conn->lock, 0);
	
	//conn->chan_list.lock = RW_LOCK_UNLOCKED();//clyu
...
/* -------- Socket interface ---------- */
static struct sock *__l2cap_get_sock_by_addr(cyg_uint16 psm, bdaddr_t *src)
{
	struct sock *sk;
 ...
 */
static struct sock *__l2cap_get_sock_by_psm(int state, cyg_uint16 psm, bdaddr_t *src)
{
...
static inline struct sock *l2cap_get_sock_by_psm(int state, cyg_uint16 psm, bdaddr_t *src)
{
	struct sock *s;
...
static void l2cap_sock_kill(struct sock *sk)
{
	...
	BT_DBG("sk %p state %d refcnt %d", sk, sk->state,sk->refcnt);

	/* Kill poor orphan */
	bluez_sock_unlink(&l2cap_sk_list, sk);
//	diag_printf("sk->refcnt=%d\n",sk->refcnt);
	sk->dead = 1;
	sock_put(sk);
}
...
static void __l2cap_sock_close(struct sock *sk, int reason)
{
	BT_DBG("sk %p state %d socket %p sk->refcnt %d", sk, sk->state, sk->socket,sk->refcnt);
	...
	BT_DBG("end sk %p state %d socket %p sk->refcnt %d", sk, sk->state, sk->socket,sk->refcnt);
}
...
static int l2cap_sock_create(struct socket *sock, int protocol)
{
	...
	if (!(sk = l2cap_sock_alloc(sock, protocol, 0)))
		return -ENOMEM;

	l2cap_sock_init(sk, NULL);
	BT_DBG("sock %p %p %d", sock, sk, sk->refcnt);
	return 0;
}
...
static int l2cap_sock_bind(struct socket *sock, struct sockaddr *addr, int addr_len)
{
	...
	BT_DBG("sk %p, %s %d, sk->socket=%p,sk->refcnt %d", sk, batostr(&la->l2_bdaddr), la->l2_psm, sk->socket,sk->refcnt);
...
int l2cap_sock_listen(struct socket *sock, int backlog)
{
	...
	BT_DBG("sk %p backlog %d sk->socket %p,sk->refcnt %d", sk, backlog, sk->socket,sk->refcnt);
...
//		cyg_semaphore_init(&sk->sem, 0);
	...
	//BT_DBG
//	printf("l2cap_accept new sk %p,%d", nsk,!skb_queue_empty(&nsk->receive_queue));
...
static int l2cap_sock_setsockopt(struct socket *sock, int level, int optname, char *optval, int optlen)
{
	...
	case L2CAP_LM:
		if (get_user(&opt, (cyg_uint32 *)optval)) {
			err = -EFAULT;
			break;
		}
...
static int l2cap_sock_getsockopt(struct socket *sock, int level, int optname, char *optval, int *optlen)
{
	...
	case L2CAP_LM:
		if (put_user(l2cap_pi(sk)->link_mode, (cyg_uint32 *)optval))
			err = -EFAULT;
		break;
...
static int l2cap_sock_shutdown(struct socket *sock, int how)
{
	...
	BT_DBG("sock %p, sk %p, sk->socket %p", sock, sk, sk->socket);
...
static int l2cap_sock_release(struct socket *sock)
{
	...
	BT_DBG("sock %p, sk %p, sk->socket %p,sk->refcnt %d", sock, sk, sk->socket,sk->refcnt);

	if (!sk) return 0;

	err = l2cap_sock_shutdown(sock, 2);
//diag_printf("l2cap_sock_release sk->refcnt=%d\n",sk->refcnt);
	sock_orphan(sk);
//diag_printf("l2cap_sock_release sk->refcnt=%d\n",sk->refcnt);
	l2cap_sock_kill(sk);
	return err;
...
/* --------- L2CAP channels --------- */
static struct sock * __l2cap_get_chan_by_dcid(struct l2cap_chan_list *l, cyg_uint16 cid)
{
	struct sock *s;
...
static struct sock *__l2cap_get_chan_by_scid(struct l2cap_chan_list *l, cyg_uint16 cid)
{
	struct sock *s;
	for (s = l->head; s; s = l2cap_pi(s)->next_c) {
 ...
 * Returns locked socket */
static inline struct sock *l2cap_get_chan_by_scid(struct l2cap_chan_list *l, cyg_uint16 cid)
{
	struct sock *s;
	read_lock(&l->lock);
...
static cyg_uint16 l2cap_alloc_cid(struct l2cap_chan_list *l)
{
	cyg_uint16 cid = 0x0040;
...
#if 0	
	printf("sk=%x,l2cap_chan_ready %x ",sk,parent);
	if(parent)
		printf("data_ready %x",parent->data_ready);
	else
		printf("sk->state_change %x",sk->state_change);
#endif
...
static int l2cap_chan_send(struct sock *sk, struct msghdr *msg, int len)
{
	...
	if (sk->type == SOCK_DGRAM)
		put_unaligned(l2cap_pi(sk)->psm, (cyg_uint16 *) skb_put(skb, 2));
...
static inline cyg_uint8 l2cap_get_ident(struct l2cap_conn *conn)
{
	cyg_uint8 id;
...
//	spin_unlock(&conn->lock);
	cyg_spinlock_clear(&conn->lock);

	return id;
}

static struct sk_buff *l2cap_build_cmd(struct l2cap_conn *conn,
				cyg_uint8 code, cyg_uint8 ident, cyg_uint16 dlen, void *data)
{
	struct sk_buff *skb, **frag;
	l2cap_cmd_hdr *cmd;
	l2cap_hdr *lh;
...
static int l2cap_send_req(struct l2cap_conn *conn, cyg_uint8 code, cyg_uint16 len, void *data)
{
	cyg_uint8 ident = l2cap_get_ident(conn);
...
static inline int l2cap_get_conf_opt(void **ptr, int *type, int *olen, unsigned long *val)
{
	...
	*olen = opt->len;
	...
	switch (opt->len) {
	case 1:
		*val = *((cyg_uint8 *) opt->val);
		break;

	case 2:
		*val = __le16_to_cpu(*((cyg_uint16 *)opt->val));
		break;

	case 4:
		*val = __le32_to_cpu(*((cyg_uint32 *)opt->val));
		break;
...
static void l2cap_add_conf_opt(void **ptr, cyg_uint8 type, cyg_uint8 len, unsigned long val)
{
	...
	switch (len) {
	case 1:
		*((cyg_uint8 *) opt->val)  = val;
		break;

	case 2:
		*((cyg_uint16 *) opt->val) = __cpu_to_le16(val);
		break;

	case 4:
		*((cyg_uint32 *) opt->val) = __cpu_to_le32(val);
		break;
	...
	(char*)*ptr += L2CAP_CONF_OPT_SIZE + len;
}
...
//inline
static int l2cap_connect_req(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
	...
	cyg_uint16 dcid = 0, scid = __le16_to_cpu(req->scid);
	cyg_uint16 psm  = req->psm;
...
//	printf("l2cap_connect_req:psm 0x%2.2x scid 0x%4.4x", psm, scid);
	...
	sk = l2cap_sock_alloc(NULL, BTPROTO_L2CAP, 0);
	if (!sk)
		goto response;
...
//inline
static  int l2cap_connect_rsp(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
	l2cap_conn_rsp *rsp = (l2cap_conn_rsp *) data;
	cyg_uint16 scid, dcid, result, status;
	struct sock *sk;
	char req[128];
	...
	//BT_DBG
//	printf("l2cap_connect_rsp:dcid 0x%4.4x scid 0x%4.4x result 0x%2.2x status 0x%2.2x", dcid, scid, result, status);
...
static inline int l2cap_config_req(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
	l2cap_conf_req * req = (l2cap_conf_req *) data;
	cyg_uint16 dcid, flags;
	cyg_uint8 rsp[64];
	struct sock *sk;
	int result;
...
//printf("l2cap_config_req:l2cap_pi(sk)->conf_state = %d,%x\n",l2cap_pi(sk)->conf_state,(l2cap_pi(sk)->conf_state));
...
//inline
static  int l2cap_config_rsp(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
	l2cap_conf_rsp *rsp = (l2cap_conf_rsp *)data;
	cyg_uint16 scid, flags, result;
	struct sock *sk;
	int err = 0;
	...
	//BT_DBG
	//printf("config rsp scid 0x%4.4x flags 0x%2.2x result 0x%2.2x", scid, flags, result);
...
//printf("l2cap_config_rsp l2cap_pi(sk)->conf_state %d %x\n",l2cap_pi(sk)->conf_state,&(l2cap_pi(sk)->conf_state));
...
static inline int l2cap_disconnect_req(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
	l2cap_disconn_req *req = (l2cap_disconn_req *) data;
	l2cap_disconn_rsp rsp;
	cyg_uint16 dcid, scid;
	struct sock *sk;
...
static inline int l2cap_disconnect_rsp(struct l2cap_conn *conn, l2cap_cmd_hdr *cmd, cyg_uint8 *data)
{
...
static inline void l2cap_sig_channel(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	while (len >= L2CAP_CMD_HDR_SIZE) {
		...
		default:
			//BT_ERR
			printf("%s:Unknown signaling command 0x%2.2x\n", __func__,cmd.code);
			err = -EINVAL;
			break;
		};
...
//inline
static int l2cap_data_channel(struct l2cap_conn *conn, cyg_uint16 cid, struct sk_buff *skb)
{
	struct sock *sk;
	...
	if (!sk) {
		//BT_DBG
		printf("l2cap_data_channel:unknown cid 0x%4.4x", cid);
		goto drop;
	}

	//BT_DBG
	//printf("l2cap_data_channel:dead %d, state %x, len %d, data %x, imtu %d ", sk->dead, sk->state, skb->len, skb->data, l2cap_pi(sk)->imtu);
	//printf("rmem_alloc = %d,skb->truesize = %d, rcvbuf = %d",atomic_read(&sk->rmem_alloc), skb->truesize,(unsigned)sk->rcvbuf);
	//printf("&sk->receive_queue=%x,sk->dead=%d,sk->state=%d,l2cap_pi(sk)->imtu=%d ",&sk->receive_queue,sk->dead,sk->state,l2cap_pi(sk)->imtu);
	printf("l2cap_data_channel");
...
static inline int l2cap_conless_channel(struct l2cap_conn *conn, cyg_uint16 psm, struct sk_buff *skb)
{
	struct sock *sk;
...
static void l2cap_recv_frame(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	//BT_DBG
//	printf("len %d, cid 0x%4.4x", len, cid);
printf("l2cap_recv_frame:%d",cid);
	...
	case 0x0002:
		psm = get_unaligned((cyg_uint16 *) skb->data);
		skb_pull(skb, 2);
...
static int l2cap_connect_ind(struct hci_dev *hdev, bdaddr_t *bdaddr, cyg_uint8 type)
{
	...
	for (sk = l2cap_sk_list.head; sk; sk = sk->next) {
		...
		if (!bacmp(&bluez_pi(sk)->src, &hdev->bdaddr)) {
			...
			exact++;
		} else if (!bacmp(&bluez_pi(sk)->src, &ba2))
...
static int l2cap_connect_cfm(struct hci_conn *hcon, cyg_uint8 status)
{
...
static int l2cap_disconn_ind(struct hci_conn *hcon, cyg_uint8 reason)
{
...
static int l2cap_auth_cfm(struct hci_conn *hcon, cyg_uint8 status)
{
...
static int l2cap_encrypt_cfm(struct hci_conn *hcon, cyg_uint8 status)
{
	struct l2cap_chan_list *l;
	struct l2cap_conn *conn;
	l2cap_conn_rsp rsp;
...
static int l2cap_recv_acldata(struct hci_conn *hcon, struct sk_buff *skb, cyg_uint16 flags)
{
	...
	//BT_DBG("conn %p len %d flags 0x%x data 0x%x", conn, skb->len, flags,skb->data);
	printf("l2cap_recv_acldata:%x",flags);
	...
	if (flags & ACL_START) {
		...
		if (conn->rx_len) {
			//BT_ERR
			printf("%s:Unexpected start frame (len %d)\n", __func__,skb->len);
			kfree_skb(conn->rx_skb);
			conn->rx_skb = NULL;
			conn->rx_len = 0;
		...
		if (skb->len < 2) {
			//BT_ERR
			printf("%s:Frame is too short (len %d)\n",__func__, skb->len);
			l2cap_conn_unreliable(conn, ECOMM);
			goto drop;
		}
		...
		conn->rx_skb = bluez_skb_alloc(len, 0);
		if (!conn->rx_skb)
			goto drop;
		...
		if (!conn->rx_len) {
			//BT_ERR
			printf("%s:Unexpected continuation frame (len %d)\n",__func__, skb->len);
			l2cap_conn_unreliable(conn, ECOMM);
			goto drop;
		}
...
int l2cap_init(void)
{
	...
	if ((err = bluez_sock_register(BTPROTO_L2CAP, &l2cap_sock_family_ops))) {
		//BT_ERR
		printf("%s:Can't register L2CAP socket\n",__func__);
		return err;
	}

	if ((err = hci_register_proto(&l2cap_hci_proto))) {
		//BT_ERR
		printf("%s:Can't register L2CAP protocol\n",__func__);
		return err;
	}
	...
	//BT_INFO
	printf("BlueZ L2CAP ver %s Copyright (C) 2000,2001 Qualcomm Inc\n", VERSION);
	//BT_INFO
...
void l2cap_cleanup(void)
{
	...
	if (bluez_sock_unregister(BTPROTO_L2CAP))
		//BT_ERR
		printf("%s:Can't unregister L2CAP socket\n",__func__);

	if (hci_unregister_proto(&l2cap_hci_proto))
		//BT_ERR
		printf("Can't unregister L2CAP protocol\n",__func__);
}
