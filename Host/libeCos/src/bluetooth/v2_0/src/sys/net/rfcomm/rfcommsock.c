...
static void rfcomm_sk_state_change(struct rfcomm_dlc *d, int err)
{
	...
	//BT_DBG
//	printf("sk %x dlc %p state %ld err %d=%x", sk, d, d->state, err,__builtin_return_address(0));
...
/* ---- Socket functions ---- */
static struct sock *__rfcomm_get_sock_by_addr(cyg_uint8 channel, bdaddr_t *src)
{
	struct sock *sk;
 ...
 */
static struct sock *__rfcomm_get_sock_by_channel(int state, cyg_uint8 channel, bdaddr_t *src)
{
...
static inline struct sock *rfcomm_get_sock_by_channel(int state, cyg_uint8 channel, bdaddr_t *src)
{
	struct sock *s;
...
//	printf("rfcomm_sock_create %p\n", sock);
	...
	if (!(sk = rfcomm_sock_alloc(sock, protocol, 0)))
		return -ENOMEM;
...
//		cyg_semaphore_init(&sk->sem, 0);
		...
		cyg_semaphore_timed_wait(&sk->sem, sleep_time);
		timeo = 0;
		lock_sock(sk);
	...
	//BT_DBG
//	printf("rfcomm_accept new sk %p,%d", nsk,!skb_queue_empty(&nsk->receive_queue));
			       ...
			       int flags, struct scm_cookie *scm)
{
	...
	BT_DBG("rfcomm sk %p size %d\n", sk, size);

	lock_sock(sk);

	target = sock_rcvlowat(sk, flags & MSG_WAITALL, size);
	timeo  = sock_rcvtimeo(sk, flags & MSG_DONTWAIT);
//printf("rfcomm_sock_recvmsg:sock=%x,sk=%x",sock,sk);
	do {
		struct sk_buff *skb;
		int chunk;
...
//printf("skb=%x,skb->len=%d,size=%d,chunk=%d;",skb,skb->len,size,chunk);
...
int rfcomm_connect_ind(struct rfcomm_session *s, cyg_uint8 channel, struct rfcomm_dlc **d)
{
	...
	//BT_DBG
//	printf("rfcomm_connect_ind:session %p channel %d", s, channel);
	...
	if (!parent)
		return 0;
//printf("parent->ack_backlog=%d,parent->max_ack_backlog=%d ",parent->ack_backlog,parent->max_ack_backlog);
	...
	sk = rfcomm_sock_alloc(NULL, BTPROTO_RFCOMM, 0);
	if (!sk)
		goto done;
//printf("rfcomm_sock_alloc:sk=%x,socket=%x",sk,parent->socket);
...
int rfcomm_init_sockets(void)
{
	...
	if ((err = bluez_sock_register(BTPROTO_RFCOMM, &rfcomm_sock_family_ops))) {
		//BT_ERR
		printf("Can't register RFCOMM socket layer\n");
		return err;
	}
...
void rfcomm_cleanup_sockets(void)
{
	...
	if ((err = bluez_sock_unregister(BTPROTO_RFCOMM)))
		//BT_ERR
		printf("Can't unregister RFCOMM socket layer %d\n", err);
}
#endif
