...
static void hci_conn_init_timer(struct hci_conn *conn)
{
    ...
    cyg_alarm_create(h, (cyg_alarm_t *)hci_conn_timeout, (cyg_addrword_t)conn, &conn->timeout_alarm_handle, &conn->timeout_alarm);
...
struct hci_conn *hci_conn_add(struct hci_dev *hdev, int type, bdaddr_t *dst)
{
	...
	if (!(conn = malloc(sizeof(struct hci_conn))))
		return NULL;
...
{
	bdaddr_t tmp = BDADDR_ANY;//clyu add tmp variable
	int use_src = bacmp(src, &tmp);
