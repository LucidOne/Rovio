...
void hci_send_to_sock(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	for (sk = hci_sk_list.head; sk; sk = sk->next) {
		...
		if (skb->pkt_type == HCI_EVENT_PKT) {
			register int evt = (*(cyg_uint8 *)skb->data & HCI_FLT_EVENT_BITS);

			if (!hci_test_bit(evt, &flt->event_mask))
				continue;

			if (flt->opcode && ((evt == EVT_CMD_COMPLETE && 
					flt->opcode != *(cyg_uint16 *)(skb->data + 3)) ||
					(evt == EVT_CMD_STATUS && 
					flt->opcode != *(cyg_uint16 *)(skb->data + 4))))
				continue;
		}
...
//diag_printf("data[0]=%x %x %x %x\n",nskb->data[0],nskb->data[1],nskb->data[2],nskb->data[3]);
//printf("sk=%x,skb=%x, nskb=%x, len=%d %d, %x\n",sk,skb,nskb, skb->len,nskb->len, nskb->data);
...
static int hci_sock_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	BT_DBG("cmd %x arg %lx HCIGETDEVLIST %x HCIDEVUP %x", cmd, arg, HCIGETDEVLIST,HCIDEVUP);
...
printf("sizeof(struct sockaddr_hci)=%d,haddr->hci_family=%d,%d",sizeof(struct sockaddr_hci),haddr->hci_family,haddr->hci_len);
...
{
	cyg_uint32 mask = hci_pi(sk)->cmsg_mask;
...
//diag_printf("len=%d, skb=%x %x,data[0]=%x %x %x %x\n",skb->len,skb,skb->data,skb->data[0],skb->data[1],skb->data[2],skb->data[3]);
                            ...
                            struct scm_cookie *scm)
{
	...
	if (skb->pkt_type == HCI_COMMAND_PKT) {
		cyg_uint16 opcode = __le16_to_cpu(get_unaligned((cyg_uint16 *)skb->data));
		cyg_uint16 ogf = cmd_opcode_ogf(opcode);
		cyg_uint16 ocf = cmd_opcode_ocf(opcode);
...
{
	struct sock *sk = sock->sk;
	struct hci_filter flt = { 0,0,/*opcode:*/ 0 };
	int err = 0, opt = 0;
...
static int hci_sock_create(struct socket *sock, int protocol)
{
	...
	if (!(sk = sk_alloc(PF_BLUETOOTH, 0, 1)))
		return -ENOMEM;
...
int hci_sock_init(void)
{
	if (bluez_sock_register(BTPROTO_HCI, &hci_sock_family_ops)) {
		//BT_ERR
		printf("%s:Can't register HCI socket\n",__func__);
		return -EPROTO;
	}
...
int hci_sock_cleanup(void)
{
	if (bluez_sock_unregister(BTPROTO_HCI))
		//BT_ERR
		printf("%s:Can't unregister HCI socket\n",__func__);
