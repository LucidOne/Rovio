...
static void hci_alarm_timeout_init( unsigned long hdev );
...
static cyg_handle_t rx_alarm_handle,tx_alarm_handle,cmd_alarm_handle;
static cyg_alarm rx_alarm;
static cyg_alarm tx_alarm;
static cyg_alarm cmd_alarm;
cyg_flag_t hci_rxtxcmd_flag; 

#define STACK_SIZE (CYGNUM_HAL_STACK_SIZE_TYPICAL + 4096)
static char hci_alarm_stack[STACK_SIZE];
static cyg_thread hci_alarm_thread_data;
cyg_handle_t hci_alarm_thread_handle;
/* ---- end ---- */
...
static int __hci_request(struct hci_dev *hdev, void (*req)(struct hci_dev *hdev, unsigned long opt), unsigned long opt, cyg_uint32 timeout)
{
	int err = 0;
	...
	//	return -EINTR;
	cyg_semaphore_timed_wait(&hdev->sem, timeout + cyg_current_time());
...
static inline int hci_request(struct hci_dev *hdev, void (*req)(struct hci_dev *hdev, unsigned long opt),
                                  unsigned long opt, cyg_uint32 timeout)
{
	int ret;
...
void inquiry_cache_update(struct hci_dev *hdev, inquiry_info *info)
{
	...
	if (!(e = inquiry_cache_lookup(hdev, &info->bdaddr))) {
		...
		if (!(e = malloc(sizeof(struct inquiry_entry))))
			return;
	...
	e->timestamp = cyg_current_time();//jiffies;
	cache->timestamp = cyg_current_time();//jiffies;
}
...
int hci_inquiry(unsigned long arg)
{
	 ...
	 */
	if (!(buf = malloc(sizeof(inquiry_info) * max_rsp))) {
		err = -ENOMEM;
		goto done;
	}
...
//diag_printf("ret = %d\n",ret);
	if (!ret) {
		...
		//cyg_thread_kill(hci_alarm_thread_handle);
...
int hci_dev_reset(cyg_uint16 dev)
{
	...
	if (!(hdev = hci_dev_get(dev)))
		return -ENODEV;
//diag_printf("hci_dev_reset\n");
	hci_req_lock(hdev);
	if(hci_alarm_thread_handle != cyg_thread_self())
		cyg_thread_suspend(hci_alarm_thread_handle);
//	tasklet_disable(&hdev->tx_task);
	...
	if(hci_alarm_thread_handle != cyg_thread_self())
		cyg_thread_resume(hci_alarm_thread_handle);
	hci_req_unlock(hdev);
	hci_dev_put(hdev);
	return ret;
}
...
int hci_dev_cmd(unsigned int cmd, unsigned long arg)
{
	...
	case HCISETPTYPE:
		hdev->pkt_type = (cyg_uint16) dr.dev_opt;
		break;
		
	case HCISETLINKPOL:
		hdev->link_policy = (cyg_uint16) dr.dev_opt;
		break;

	case HCISETLINKMODE:
		hdev->link_mode = ((cyg_uint16) dr.dev_opt) & (HCI_LM_MASTER | HCI_LM_ACCEPT);
		break;

	case HCISETACLMTU:
		hdev->acl_mtu  = *((cyg_uint16 *)&dr.dev_opt + 1);
		hdev->acl_pkts = *((cyg_uint16 *)&dr.dev_opt + 0);
		break;

	case HCISETSCOMTU:
		hdev->sco_mtu  = *((cyg_uint16 *)&dr.dev_opt + 1);
		hdev->sco_pkts = *((cyg_uint16 *)&dr.dev_opt + 0);
		break;
...
int hci_get_dev_list(unsigned long arg)
{
	...
	if (get_user((int*)&dev_num, /*(cyg_uint16 *)*/ (int*)arg))
		return -EFAULT;
...
int hci_register_dev(struct hci_dev *hdev)
{
	...
	cyg_spinlock_init(&hdev->lock, 0);//default unlock
	
	hdev->flags = 0;
	hdev->pkt_type  = (HCI_DM1 | HCI_DH1 | HCI_HV1);
	hdev->link_mode = (HCI_LM_ACCEPT);
	cyg_semaphore_init(&hdev->sem, 0);
	...
	hci_alarm_timeout_init((unsigned long)hdev);
...
static int hci_send_frame(struct sk_buff *skb)
{
	...
	if (atomic_read(&hdev->promisc)) {
		/* Time stamp */
	        do_gettimeofday(&skb->stamp);
//diag_printf("Time stamp hci_send_to_sock\n");
...
int hci_send_cmd(struct hci_dev *hdev, cyg_uint16 ogf, cyg_uint16 ocf, cyg_uint32 plen, void *param)
{
	...
	BT_DBG("%s ogf 0x%x ocf 0x%x plen %d\n", hdev->name, ogf, ocf, plen);

	if (!(skb = bluez_skb_alloc(len, 0))) {
		//BT_ERR
		printf("%s Can't allocate memory for HCI command\n", hdev->name);
		return -ENOMEM;
	}
...
/* Send ACL data */
static void hci_add_acl_hdr(struct sk_buff *skb, cyg_uint16 handle, cyg_uint16 flags)
{
	int len = skb->len;
	hci_acl_hdr *ah;
...
//		spin_lock_bh(&conn->data_q.lock);
		cyg_spinlock_spin(&conn->data_q.lock);
...
//		spin_unlock_bh(&conn->data_q.lock);
		cyg_spinlock_clear(&conn->data_q.lock);
	}
...
static inline struct hci_conn *hci_low_sent(struct hci_dev *hdev, cyg_uint8 type, int *quote)
{
...
static inline void hci_sched_acl(struct hci_dev *hdev)
{
	...
	if (!hdev->acl_cnt && (cyg_current_time() - hdev->acl_last_tx) > (HZ * 45))
		hci_acl_tx_to(hdev);
	...
	while (hdev->acl_cnt && (conn = hci_low_sent(hdev, ACL_LINK, &quote))) {
		while (quote-- && (skb = skb_dequeue(&conn->data_q))) {
			...
			hci_send_frame(skb);
			hdev->acl_last_tx = cyg_current_time();//jiffies;
...
static inline void hci_acldata_packet(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	//BT_DBG
	//printf("%s len %d handle 0x%x flags 0x%x", hdev->name, skb->len, handle, flags);
	printf("hci_acldata_packet:%d",skb->len);
...
static void hci_cmd_task(unsigned long arg)
{
	...
	//BT_DBG("hci_cmd_task:%s cmd %d", hdev->name, atomic_read(&hdev->cmd_cnt));

	if (!atomic_read(&hdev->cmd_cnt) && (cyg_current_time() - hdev->cmd_last_tx) > HZ) {
		diag_printf("%s command tx timeout", hdev->name);
		atomic_set(&hdev->cmd_cnt, 1);
	}
	...
	if (atomic_read(&hdev->cmd_cnt) && (skb = skb_dequeue(&hdev->cmd_q))) {
		...
		if ((hdev->sent_cmd = skb_clone(skb, 0))) {
			atomic_dec(&hdev->cmd_cnt);
			hci_send_frame(skb);
			hdev->cmd_last_tx = cyg_current_time();//jiffies;
		} else {
...
static void hci_alarm_thread(cyg_addrword_t param)
{
    while ( 1 ) {
        volatile int x;
//printf("hci_alarm_thread:%d",param);
        x = cyg_flag_wait(
            &hci_rxtxcmd_flag,
            -1,
...
static void hci_alarm_timeout_init( unsigned long hdev )
{
    ...
    //cyg_alarm_create(h, hci_rx_task, 0, &rx_alarm_handle, &rx_alarm);
    //cyg_alarm_create(h, hci_tx_task, 0, &tx_alarm_handle, &tx_alarm);
    //cyg_alarm_create(h, hci_cmd_task, 0, &cmd_alarm_handle, &cmd_alarm);
    // Init the flag of waking up
    cyg_flag_init( &hci_rxtxcmd_flag );

    // Create alarm background thread to run the callbacks
    cyg_thread_create(
        CYGPKG_BLUEZ_FAST_THREAD_PRIORITY, // Priority
        hci_alarm_thread,                   // entry
        (cyg_addrword_t)hdev,                              // entry parameter
        "hci alarm support",        // Name
        &hci_alarm_stack[0],                // Stack
        STACK_SIZE,                     // Size
        &hci_alarm_thread_handle,           // Handle
        &hci_alarm_thread_data              // Thread data structure
        );
    cyg_thread_resume(hci_alarm_thread_handle);    // Start it
}
