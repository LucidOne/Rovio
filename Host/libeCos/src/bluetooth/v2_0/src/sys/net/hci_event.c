...
static void hci_cc_link_ctl(struct hci_dev *hdev, cyg_uint16 ocf, struct sk_buff *skb)
{
	cyg_uint8 status;
...
static void hci_cc_link_policy(struct hci_dev *hdev, cyg_uint16 ocf, struct sk_buff *skb)
{
...
static void hci_cc_host_ctl(struct hci_dev *hdev, cyg_uint16 ocf, struct sk_buff *skb)
{
	cyg_uint8 status, param;
	void *sent;
...
static void hci_cc_info_param(struct hci_dev *hdev, cyg_uint16 ocf, struct sk_buff *skb)
{
...
static inline void hci_cs_create_conn(struct hci_dev *hdev, cyg_uint8 status)
{
	struct hci_conn *conn;
	...
	if (status) {
		...
		if (!conn) {
			...
			if (conn) {
				...
				//BT_ERR
				printf("No memmory for new connection\n");
		}
	}
...
static void hci_cs_link_ctl(struct hci_dev *hdev, cyg_uint16 ocf, cyg_uint8 status)
{
	BT_DBG("hci_cs_link_ctl: %s ocf 0x%x", hdev->name, ocf);
...
static void hci_cs_link_policy(struct hci_dev *hdev, cyg_uint16 ocf, cyg_uint8 status)
{
	BT_DBG("%s ocf 0x%x", hdev->name, ocf);
...
static void hci_cs_host_ctl(struct hci_dev *hdev, cyg_uint16 ocf, cyg_uint8 status)
{
	BT_DBG("%s ocf 0x%x", hdev->name, ocf);
...
static void hci_cs_info_param(struct hci_dev *hdev, cyg_uint16 ocf, cyg_uint8 status)
{
...
{
	cyg_uint8 status = *((cyg_uint8 *) skb->data);
...
static inline void hci_inquiry_result_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	int num_rsp = *((cyg_uint8 *) skb->data);
...
static inline void hci_disconn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	struct hci_conn *conn = NULL;
	cyg_uint16 handle = __le16_to_cpu(dc->handle);
...
static inline void hci_num_comp_pkts_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	for (i = 0, ptr = (cyg_uint16 *) skb->data; i < nc->num_hndl; i++) {
		struct hci_conn *conn;
		cyg_uint16  handle, count;
...
static inline void hci_auth_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	struct hci_conn *conn = NULL;
	cyg_uint16 handle = __le16_to_cpu(ac->handle);
...
static inline void hci_encrypt_change_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	struct hci_conn *conn = NULL;
	cyg_uint16 handle = __le16_to_cpu(ec->handle);
