...
static cyg_devio_table_t tty_devio = {
                   tty_write,
                   tty_read,
                   cyg_devio_bwrite,
                   cyg_devio_bread,
...
cyg_devtab_entry_t tty_io_diag = {
//             "/dev/console",       
//             CYGDAT_IO_SERIAL_TTY_CONSOLE,   // Based on driver for this device
	...
	|| defined IPCAM_CONFIG_IP_CAM_VER_3
             "/dev/ser0",//"/dev/ser1",//"/dev/haldiag",//"/dev/ser0",
...
#if 1	//xhchen
			int uart_is_sys_msg_suppressed(void);
			void vcom_write(char *buf, size_t size);
			
			if (uart_is_sys_msg_suppressed())
				res = ENOERR;
			else
#endif
            res = cyg_io_write(chan, xbuf, (cyg_uint32 *)&size);
            vcom_write((char *)xbuf, size);
