...
static Cyg_ErrNo termios_ioctl(cyg_io_handle_t handle, cyg_uint32 cmd, const void *buf);
...
//struct cyg_devtab_entry *termios;
...
static Cyg_ErrNo termios_ioctl(cyg_io_handle_t handle, cyg_uint32 cmd, const void *buf)
{
    ...
    cyg_io_handle_t chan = (cyg_io_handle_t)priv->dev_handle;//chan = &w90n740_serial_io0
	
	err = cyg_io_ioctl(chan, cmd, buf);
	//diag_printf("termios_ioctl err=%d\n",err);
	return err;
#else
	return ENOSYS;
#endif
...
termios_write(cyg_io_handle_t handle, const void *_buf, cyg_uint32 *len)
{
    ...
    while (input_bytes_read++ < *len) {
            ...
            (*buf == '\n'))
        {
            ...
            res = cyg_io_write(chan, xbuf, (cyg_uint32*)&size);
            if (res != ENOERR) {
