...
static Cyg_ErrNo serial_ioctl(cyg_io_handle_t handle, cyg_uint32 key, const void *buf);
...
void *serial_getdisc(cyg_io_handle_t handle)
{
	cyg_devtab_entry_t *t = (cyg_devtab_entry_t *)handle;
    struct serial_channel *priv = (struct serial_channel *)t->priv;
   // diag_printf("getdisc priv=%x, &termios_private_info0=%x\n",priv,&termios_private_info0);
   
    return priv->disc_data;
}

void serial_setdisc(cyg_io_handle_t handle, void* disc)
{
	cyg_devtab_entry_t *t = (cyg_devtab_entry_t *)handle;
    struct serial_channel *priv = (struct serial_channel *)t->priv;
    //diag_printf("setdisc disc=%x, priv=%x, &termios_private_info0=%x\n",disc,priv,&termios_private_info0);
...
static Cyg_ErrNo serial_ioctl(cyg_io_handle_t handle, cyg_uint32 key, const void *buf)
{
    ...
    serial_channel *chan = (serial_channel *)t->priv;
    struct tty_ldisc *ldisc = chan->tty_ldisc;
    cyg_uint32	*i = ( cyg_uint32	*)buf;
	
//	diag_printf("serial_ioctl key = %x %x ldisc=%x, g_ldisc=%x\n", key,*(unsigned int*)buf, ldisc, g_ldisc);
//	diag_printf("chan = %x, serial = %x\n",chan,serial_driver);
	...
	switch(key)	{
		case TIOCSETD:
			if(*i == N_HCI)
			{
				if(g_ldisc)
				{
					chan->tty_ldisc = g_ldisc;

//diag_printf("TIOCSETD ldisc=%x %x\n",ldisc,ldisc->open);
			    	if(g_ldisc && g_ldisc->open)
    					g_ldisc->open(serial_driver);
			...
			break;
		default:
    		if(ldisc && ldisc->ioctl)
    		{
    			if(ldisc->ioctl(serial_driver, NULL, key, (cyg_uint32)buf) >= 0)
    				return err;
    			else
    				return -ENOENT;
    		}	
