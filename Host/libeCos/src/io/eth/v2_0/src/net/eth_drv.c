...
// DEBUG
	...
	case SIOCSIFSENDPACKET:
	{
		extern eth_packet_send(
			struct arpcom *ac,
		...
		struct iwreq *req = (struct iwreq *)ifa;
		
		struct ether_header *e_hdr = (struct ether_header *)req->u.data.pointer;

		const char *buffer = *(char **)(req->u.data.pointer + ALIGN(sizeof(*e_hdr)));
		size_t length = *(size_t *)(req->u.data.pointer + ALIGN(sizeof(*e_hdr)) + sizeof(char *));
		
		diag_printf("e_hdr = %x, buffer =%x, length=%d\n", e_hdr, buffer, length);
		
		eth_packet_send(&sc->sc_arpcom,
			e_hdr,
			buffer,
			length);
		break;
    ...
    default:
    	...
    	//diag_printf("cmd=%x[%x,%x]\n", (int)cmd, (int)SIOCIWFIRST, (int)SIOCIWLAST);
		if ((cmd & ~IOC_DIRMASK) >= (SIOCIWFIRST & ~IOC_DIRMASK) && (cmd & ~IOC_DIRMASK) <= (SIOCIWLAST & ~IOC_DIRMASK)) {
			int ret;
			...
			ret = wireless_process_ioctl(ifp, cmd, data);
			//rtnl_unlock();
			#if 0//clyu
			if (!ret && IW_IS_GET(cmd)/* &&
			    copy_to_user(data, &ifr, sizeof(struct ifreq))*/)
				error = EINVAL;//return -EFAULT;
			#endif
			if(ret != 0)
				error = -ret;
		}
...
eth_drv_recv(struct eth_drv_sc *sc, int total_len)
{
	...
	//diag_printf("eth_drv_recv, len=%d\n", total_len);
    ...
    while (total_len > 0) {
        ...
        total_len -= mlen;
        data = (unsigned char *)mtod(m, caddr_t);
        sg_list[sg_len].buf = (CYG_ADDRESS)data;
