...
#endif
#ifdef CYGHWR_NET_DRIVER_WLAN0
struct dhcp_lease wlan0_lease = { &dhcp_needs_attention, 0,0,0,0,0,0 };
#endif
...
int dhcp_bind( void )
{
...
#endif
#ifdef CYGHWR_NET_DRIVER_WLAN0
    cyg_uint8 old_wlan0_dhcpstate = wlan0_dhcpstate;
#endif
...
#ifdef CYGHWR_NET_DRIVER_WLAN0
    if (wlan0_up
        && DHCPSTATE_FAILED != wlan0_dhcpstate )
            wlan0_up = do_dhcp(wlan0_name, &wlan0_bootp_data, &wlan0_dhcpstate, &wlan0_lease);
#endif           
    ...
    if ( wlan0_up
         ...
         && old_wlan0_dhcpstate != wlan0_dhcpstate ) {
...
#endif
#ifdef CYGHWR_NET_DRIVER_WLAN0
    if ( old_wlan0_dhcpstate == DHCPSTATE_BOUND &&
         wlan0_dhcpstate == DHCPSTATE_NOTBOUND )
        return 0; // a lease timed out; we became unbound
#endif
...
#define STACK_SIZE (4096 + sizeof(struct bootp))//(CYGNUM_HAL_STACK_SIZE_TYPICAL + sizeof(struct bootp))//clyu 20070615
