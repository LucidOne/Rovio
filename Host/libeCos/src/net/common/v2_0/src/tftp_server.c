		 ...
		 struct sockaddr_in *from_addr, int from_len)
{
    ...
    sendto(s, ( const void *)reply, 4+strlen(reply->th_msg)+1, 0, 
...
#endif
            sendto(s, ( const void *)reply, 4, 0, (struct sockaddr *)from_addr, from_len);
        repeat_select:
            ...
            if (ntohs(response->th_opcode) == DATA &&
                ...
                len = (server->ops->write)(fd, (void*)response->th_data, data_len-4);
                if (len < (data_len-4)) {
                    // File is "full"
                ...
                struct sockaddr_in *from_addr, int from_len)
{
    ...
    while (ok) {
        ...
        len = (server->ops->read)(fd,(void*) reply->th_data, SEGSIZE);
...
#endif
            if (sendto(s,( const void *) reply, 4+len, 0,
