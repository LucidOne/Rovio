...
do_bootp(const char *intf, struct bootp *recv)
{
    ...
    memset((void *)&addrp->sin_addr, 255, sizeof(addrp->sin_addr));
    ...
    if (recvfrom(s, recv, sizeof(struct bootp), 0,
                 (struct sockaddr *)&bootp_server_addr, (socklen_t *)&addrlen) < 0) {
    ...
    if (bp->bp_vend[0]) {
        ...
        while (*op != TAG_END) {
            ...
            case TAG_DOMAIN_SERVER:
                ...
                if (*op == TAG_SUBNET_MASK)   ap =  (unsigned char *)"  subnet mask";
                if (*op == TAG_GATEWAY)       ap =  (unsigned char *)"      gateway";
                if (*op == TAG_IP_BROADCAST)  ap =  (unsigned char *)" IP broadcast";
                if (*op == TAG_DOMAIN_SERVER) ap =  (unsigned char *)"domain server";
                diag_printf("      %s: ", ap);
                ap = (unsigned char *)&addr[0];
                while (len > 0) {
            ...
            case TAG_HOST_NAME:
                ...
                name[*(op+1)] = '\0';
                if (*op == TAG_DOMAIN_NAME) ap =  (unsigned char *)" domain name";
                if (*op == TAG_HOST_NAME)   ap =  (unsigned char *)"   host name";
                diag_printf("       %s: %s\n", ap, name);
                break;
...
init_net(const char *intf, struct bootp *bp)
{
    ...
    //clyu 20070806 remove old ip address
    ...
    if (get_bootp_option(bp, TAG_SUBNET_MASK, (void*)&addrp->sin_addr,&length)) {
    ...
    length = sizeof(addrp->sin_addr);    
    if (get_bootp_option(bp, TAG_IP_BROADCAST, (void*)&addrp->sin_addr,&length)) {
        if (ioctl(s, SIOCSIFBRDADDR, &ifr)) {
            perror("SIOCSIFBRDADDR");
            close(s);
            return false;
    ...
    // Set up routing
    length = sizeof(addrp->sin_addr);
    if (get_bootp_option(bp, TAG_GATEWAY, (void*)&gateway,&length)) {
        ...
        if ( 0 != gateway.s_addr ) {
			...
			//set_sockaddr((struct sockaddr_in *) &route.rt_gateway, gateway.s_addr, 0);

			route.rt_flags = RTF_UP | RTF_GATEWAY;
			route.rt_dev = ifr.ifr_name;
			

			if ((ret = ioctl(s,SIOCDELRT, &route)) < 0)
			{
				diag_printf("Cannot delete default route (%d %d).\n", errno, ESRCH);
			}
			//end
