...
struct bootp wlan0_bootp_data;
cyg_bool_t   wlan0_up = false;
...
void init_all_network_interfaces(void)
{
	...
	//diag_printf("init_all_network_interfaces return %x\n",__builtin_return_address(0));
    ...
    if ( ! eth0_up ) { // Make this call idempotent
...
#endif
        {
			...
			diag_printf("BOOTP/DHCP success on eth0\n");
...
#ifdef CYGHWR_NET_DRIVER_WLAN0
	wlan0_up = false;//clyu 20070810
    if ( ! wlan0_up ) { // Make this call idempotent
#ifdef CYGPKG_IO_PCMCIA
        if ((t = eth_drv_netdev("wlan0")) != (cyg_netdevtab_entry_t *)NULL) {
            int tries = 0;
            while (t->status != CYG_NETDEVTAB_STATUS_AVAIL) {
                if (tries == 0) {
                    diag_printf("... Waiting for PCMCIA device 'wlan0'\n");
                } 
                if (++tries == 5) {
                    diag_printf("... Giving up on PCMCIA device 'wlan0'\n");
                    goto bail_eth0;
                }
                cyg_thread_delay(100);
            }
...
#endif // CYGPKG_IO_PCMCIA
#ifdef CYGHWR_NET_DRIVER_WLAN0_BOOTP
        // Perform a complete initialization, using BOOTP/DHCP
        wlan0_up = true;
#ifdef CYGHWR_NET_DRIVER_WLAN0_DHCP
        wlan0_dhcpstate = 0; // Says that initialization is external to dhcp
        if (do_dhcp(wlan0_name, &wlan0_bootp_data, &wlan0_dhcpstate, &wlan0_lease)) 
#else
#ifdef CYGPKG_NET_DHCP
        wlan0_dhcpstate = DHCPSTATE_BOOTP_FALLBACK;
        // so the dhcp machine does no harm if called
#endif
        if (do_bootp(wlan0_name, &wlan0_bootp_data)) 
#endif
        {
#ifdef CYGHWR_NET_DRIVER_WLAN0_BOOTP_SHOW
			diag_printf("BOOTP/DHCP success on wlan0\n");
            show_bootp(wlan0_name, &wlan0_bootp_data);
#endif
        } else {
            diag_printf("BOOTP/DHCP failed on wlan0\n");
            wlan0_up = false;
        }
#elif defined(CYGHWR_NET_DRIVER_ETH0_ADDRS_IP)
        wlan0_up = true;
        build_bootp_record(&wlan0_bootp_data,
                           wlan0_name,
                           string(CYGHWR_NET_DRIVER_WLAN0_ADDRS_IP),
                           string(CYGHWR_NET_DRIVER_WLAN0_ADDRS_NETMASK),
                           string(CYGHWR_NET_DRIVER_WLAN0_ADDRS_BROADCAST),
                           string(CYGHWR_NET_DRIVER_WLAN0_ADDRS_GATEWAY),
                           string(CYGHWR_NET_DRIVER_WLAN0_ADDRS_SERVER));
        show_bootp(wlan0_name, &wlan0_bootp_data);
#endif
#ifdef CYGPKG_IO_PCMCIA
    bail_eth0:
...
#endif // CYGHWR_NET_DRIVER_WLAN0

#ifdef CYGHWR_NET_DRIVER_ETH0
#ifndef CYGHWR_NET_DRIVER_ETH0_MANUAL
    if (eth0_up) {
    	diag_printf("Network initialization ");
        if (!init_net(eth0_name, &eth0_bootp_data)) {
            diag_printf("failed for eth0\n");
            eth0_up = false;
        }
        else
        	diag_printf("success for eth0\n");
...
#ifdef CYGHWR_NET_DRIVER_WLAN0
#ifndef CYGHWR_NET_DRIVER_WLAN0_MANUAL
    if (wlan0_up) {
    	diag_printf("Network initialization ");
        if (!init_net(wlan0_name, &wlan0_bootp_data)) {
            diag_printf("failed for wlan0\n");
            wlan0_up = false;
        }
        else
        	diag_printf("success for wlan0\n");
#ifdef CYGHWR_NET_DRIVER_WLAN0_IPV6_PREFIX
        if (!init_net_IPv6(wlan0_name, &wlan0_bootp_data, 
                           string(CYGHWR_NET_DRIVER_WLAN0_IPV6_PREFIX))) {
            diag_printf("Static IPv6 network initialization failed for wlan0\n");
            wlan0_up = false;  // ???
        }
#endif
    }
...
#ifdef CYGOPT_NET_DHCP_DHCP_THREAD//defined
...
#endif
	diag_printf("init_all_network_interfaces finished\n");
