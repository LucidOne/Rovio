...
static inline void reset_timeout( struct timeval *ptv, struct timeout_state *pstate )
{
    ...
    ptv->tv_sec = 0;//clyu tv_sec should > 0?
    ptv->tv_usec = 65536 * (2 + (timeout_random & 3)); // 0.1 - 0.3S, about
}

/*
	next_timeout return false do_dhcp will return
*/
...
static inline int next_timeout( struct timeval *ptv, struct timeout_state *pstate )
{
    ...
    if ( 0 == ptv->tv_sec )
        ...
        pstate->countdown = 0; // later fast retries clyu 2-->0
        ptv->tv_sec = 0;
    }
    return pstate->secs < 20; // If longer, too many tries... clyu 100-->20
}
...
#if 0//clyu lead to alarm list abnormal. cyg_thread_delay can't return
...
//diag_printf("do_dhcp 03 %x\n",*pstate);
//#define CYGDBG_NET_DHCP_CHATTER
        switch ( *pstate ) {
        ...
        case DHCPSTATE_SELECTING:
            ...
            if (recvfrom(s, received, sizeof(struct bootp), 0,
                         (struct sockaddr *)&rx_addr, (socklen_t *)&addrlen) < 0) {
            ...
            if ( 0 == received->bp_siaddr.s_addr ) {
                ...
                get_bootp_option( received, TAG_DHCP_SERVER_ID,
                                  (void*)&received->bp_siaddr.s_addr,
                                  &length);
            }
        ...
        case DHCPSTATE_REQUEST_RECV:
            ...
            length = sizeof(msgtype);
            if ( get_bootp_option( received, TAG_DHCP_MESS_TYPE, (void*)&msgtype,
                                   &length) ) {
