...
#define _FTP_SRV           __xstring(10.130.249.102/*172.16.19.254*/) // farmnet dns0 address
                                     
#define FTPBUFSIZE (1024 * 1024)
char ftpbuf[FTPBUFSIZE];
char ftpbuf1[FTPBUFSIZE];

void test_printf(unsigned error, const char *fmt, ...) 
{

}

void ftp_test(cyg_addrword_t p)
{
  int ret;
  ...
  CYG_TEST_INFO("Getting romfs.img from %s\n" _FTP_SRV);
  
  ret = ftp_get(_FTP_SRV,"clyu2","clyu2000",
                 "romfs.img",ftpbuf,FTPBUFSIZE,
                ftpclient_printf);
