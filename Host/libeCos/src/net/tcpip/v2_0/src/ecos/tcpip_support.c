...
//__externC cyg_netdevtab_entry_t *__NETDEVTAB__;
//__externC cyg_netdevtab_entry_t *__NETDEVTAB_END__;
cyg_netdevtab_entry_t *__NETDEVTAB__ = (cyg_netdevtab_entry_t *)&Image$$ECOS_NETDEV$$Base;
cyg_netdevtab_entry_t *__NETDEVTAB_END__ = (cyg_netdevtab_entry_t *)&Image$$ECOS_NETDEV$$Limit;
...
#define STACK_SIZE (8192)//CYGNUM_HAL_STACK_SIZE_TYPICAL //clyu for stack overwrite when this thread to send packet
...
#pragma arm section zidata = "nozero"
__align(128) static unsigned char net_mempool_area[NET_MEMPOOL_SIZE];
#pragma arm section zidata
...
#pragma arm section zidata = "nozero"
__align(128) static unsigned char net_mbufs_area[NET_MBUFS_SIZE];
#pragma arm section zidata
...
#pragma arm section zidata = "nozero"
__align(128) static unsigned char net_clusters_area[NET_CLUSTERS_SIZE];
#pragma arm section zidata

static cyg_mempool_fix net_clusters_pool;
static cyg_handle_t    net_clusters;
__align(128) static char            net_clusters_refcnt[(NET_CLUSTERS_SIZE/MCLBYTES)+1];
...
#if 0//clyu defined in bsdstring.cxx in libc library
int
...
cyg_net_init(void)
{
    ...
    cyg_thread_create(CYGPKG_NET_THREAD_PRIORITY, // Priority 7
                      cyg_netint,               // entry
                      0,                        // entry parameter
...
show_network_tables(pr_fun *pr)
{
    ...
    for (i = 1; i <= AF_MAX; i++) {
        if ((rnh = rt_tables[i]) != NULL) {
            error = rnh->rnh_walktree(rnh, _dumpentry, (void *)pr);
        }
    }
