...
static SYNCH_SAVE save_args[100];
static int save_num = 0;

void save_function(const char *func, const char *file, int line, int spl_state)
{
	cyg_scheduler_lock();
	memmove(&save_args[1], &save_args[0], sizeof(save_args) - sizeof(save_args[0]));
	strncpy(save_args[0].func, func, sizeof(save_args[0].func));
	memcpy(save_args[0].file, file + strlen(file) - sizeof(save_args[0].file), sizeof(save_args[0].file));
	save_args[0].line = line;
	save_args[0].spl_state = spl_state;
...
#define SPLXTRACE save_function(__func__, file, line, spl_state)
...
spl_any( cyg_uint32 which/*, const char *func, const char *file, int line */)
{
    ...
    //save_function(func, file, line, spl_state);
...
{
    //SPLXTRACE;
    return spl_any( SPL_IMP/*, __func__, file, line */ );
}
...
{
    //SPLXTRACE;
    return spl_any( SPL_CLOCK/*, __func__, file, line */);
}
...
{
    //SPLXTRACE;
    return spl_any( SPL_NET/*, __func__, file, line */);
}
...
cyg_splhigh(SPLXARGS)
{
    ...
    return spl_any( SPL_SOFTNET/*, __func__, file, line */);
}
...
{
    //SPLXTRACE;
    return spl_any( SPL_INTERNAL/*, __func__, file, line */);
}
...
//clyu 20070713
void cyg_ifplock(cyg_mutex_t *lock)
{
	cyg_mutex_lock(lock);
}

void cyg_ifpunlock(cyg_mutex_t *lock)
{
