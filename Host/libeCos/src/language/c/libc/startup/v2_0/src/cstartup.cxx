...
//diag_printf("cstartup.cxx cyg_iso_c_start resume\n");
...
cyg_libc_startup_dummy_constructor_class cyg_libc_startup_obj
...
externC void _exit( int status );
externC void _sys_exit(int return_code)
{
	_exit(return_code); /* endless loop */
}

externC int putchar( int c );
externC void _ttywrch(int ch)
{
	putchar(ch);
}

externC char * _sys_command_string(char *cmd, int len)
{	return 0; }


externC __value_in_regs struct __initial_stackheap __user_initial_stackheap(
        unsigned R0, unsigned SP, unsigned R2, unsigned SL)
{
    struct __initial_stackheap config;

    extern unsigned int Image$$RAM_B$$ZI$$Limit;

    config.heap_base = (unsigned int)&Image$$RAM_B$$ZI$$Limit; // NOTE: The heap base must less than 0x7000
    config.heap_limit = config.heap_base + 0x700000;
