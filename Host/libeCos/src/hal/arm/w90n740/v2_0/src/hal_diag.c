...
cyg_hal_plf_serial_init_channel(void* __ch_data)
{
    ...
    // 8-1-no parity.
    HAL_WRITE_UINT32 (COM_LCR, 0x80); /* select divisor latch registers */

	div=baudrate_div(BASE_BAUD);
    HAL_WRITE_UINT32 (COM_DLL, div&0xFF);
    HAL_WRITE_UINT32 (COM_DLM, (div>>8)&0xFF);

    HAL_WRITE_UINT32 (COM_LCR, UART_LCR_NPAR|UART_LCR_WLEN8|UART_LCR_NSB); /* none parity, 8 data bits, 1 stop bits */
    ...
    // Enable RX and TX
    HAL_WRITE_UINT32(COM_IER, UART_IER_RDI|UART_IER_THRI);
}
...
cyg_hal_plf_serial_getc_nonblock(void* __ch_data, cyg_uint8* ch)
{
    ...
    HAL_READ_UINT32(COM_LSR, stat);
    if ((stat & UART_LSR_DR) == 0)
        return false;

    HAL_READ_UINT32(COM_RX, c);
    *ch = (cyg_uint8)(c & 0xff);
...
// 1/28/2003 Winbond W90N740 doesn't have modem relate registers
...
static channel_data_t w90n740_ser_channels[1] = {
    { (cyg_uint8*)UART_BASE0, 1000, CYGNUM_HAL_INTERRUPT_UART0_RX, CYGNUM_HAL_INTERRUPT_UART0_TX },
};
...
cyg_hal_plf_serial_init(void)
{
    ...
    // Init channels
    cyg_hal_plf_serial_init_channel(&w90n740_ser_channels[0]);
    ...
    CYGACC_COMM_IF_CH_DATA_SET(*comm, &w90n740_ser_channels[0]);
