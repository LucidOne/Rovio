...
#endif
//clyu? cyg_drv_cond_t ? cyg_drv_flag_t?
    cyg_drv_cond_t           wait;
...
struct tty_ldisc {
	 ...
	 */
	int	(*open)(cyg_io_handle_t );
	void	(*close)(cyg_io_handle_t);
	void	(*flush_buffer)(cyg_io_handle_t tty);
	cyg_uint32	(*chars_in_buffer)(cyg_io_handle_t tty);
	cyg_uint32	(*read)(cyg_io_handle_t tty, struct CYG_FILE_TAG * file,
			unsigned char * buf, size_t nr);

	cyg_uint32	(*write)(cyg_io_handle_t tty, struct CYG_FILE_TAG * file,
			 const unsigned char * buf, cyg_uint32 nr);	
	int	(*ioctl)(cyg_io_handle_t tty, struct CYG_FILE_TAG * file,
			 unsigned int cmd, unsigned long arg);
//	void	(*set_termios)(cyg_io_handle_t tty, struct termios * old);
//	unsigned int (*poll)(cyg_io_handle_t, struct file *, struct poll_table_struct *);
	
	/*
	 * The following routines are called from below.
	 */
	void	(*receive_buf)(cyg_io_handle_t, const unsigned char *cp,
			       char *fp, int count);
	int	(*receive_room)(cyg_io_handle_t);
	void	(*write_wakeup)(cyg_io_handle_t);
};
void *termios_getdisc(cyg_io_handle_t handle);
void termios_setdisc(cyg_io_handle_t handle, void* disc);
int serial_register_ldisc(struct tty_ldisc *new_ldisc);
int serial_unregister_ldisc(struct tty_ldisc *new_ldisc);

//extern struct cyg_devtab_entry *termios;
extern struct cyg_devtab_entry *serial_driver;
#endif
