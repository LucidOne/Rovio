...
#if 1	//Modified by xhchen 2008-07-05, htons is a macro!!!
    dns_hdr->id = htons(id);
    id++;
#else
    dns_hdr->id = htons(id++);
...
dot_hostname(const char *hostname)
{
    ...
    if (inet_aton(hostname, &addr)) {
        hent = alloc_hent();
        if (hent) {
            memcpy(hent->h_addr_list[0], (const void *)&addr, sizeof(struct in_addr));
...
parse_answer(char * msg, short rr_type)
{
    ...
    if (dns_hdr->ancount > 0) {
        ...
        switch (rr_type) {
        case DNS_TYPE_A:
            hent->h_name = real_name(msg, (unsigned char *)qname);
            if (!hent->h_name) {
                free_hent(hent);
                return NULL;
            }
...
gethostbyaddr(const char *addr, int len, int type)
{
    ...
    len = build_query((char *)msg, hostname, DNS_TYPE_PTR);
    if (len < 0) {
    ...
    len = send_recv((char *)msg, len, sizeof(msg));
    if (len < 0) {
        cyg_drv_mutex_unlock(&dns_mutex);
        CYG_REPORT_RETVAL( NULL );
        return NULL;
    ...
    hent = parse_answer((char *)msg, DNS_TYPE_PTR);
    if (hent) {
...
gethostbyname(const char * hostname)
{
    ...
    memset(msg, 0, sizeof(msg));
    len = build_query((char *)msg, hostname, DNS_TYPE_A);
    if (len < 0) {
        cyg_drv_mutex_unlock(&dns_mutex);
        CYG_REPORT_RETVAL( NULL );
        return NULL;
    ...
    /* Decode the answer */
    hent = parse_answer((char *)msg, DNS_TYPE_A);
    if (hent) {
        cyg_drv_mutex_unlock(&dns_mutex); 
        store_hent(hent);
        CYG_REPORT_RETVAL( hent );
    ...
    if (domainname) {
        ...
        memset(msg, 0, sizeof(msg));
        len = build_query((char *)msg, name, DNS_TYPE_A);
        if (len < 0) {
            cyg_drv_mutex_unlock(&dns_mutex);
            CYG_REPORT_RETVAL( NULL );
            return NULL;
