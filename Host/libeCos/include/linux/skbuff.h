...
extern void __out_of_line_bug(int line);
...
#else
#define NET_CALLER(arg) 0/*__builtin_return_address(0)*///clyu
#endif
...
//	struct  dst_entry *dst;
	...
	cyg_uint32		priority;		/* Packet queueing priority			*/
	...
	void 		(*destructor)(struct sk_buff *);	/* Destruct function		*/
       ...
       cyg_uint32           tc_index;               /* traffic control index */
#endif
};
...
static __inline cyg_uint32 skb_queue_len(struct sk_buff_head *list_)
{
	return(list_->qlen);
}
...
{
	cyg_addrword_t flags;

//	spin_lock_irqsave(&list->lock, flags);
	cyg_spinlock_spin_intsave(&list->lock, &flags);
	__skb_queue_head(list, newsk);
//	spin_unlock_irqrestore(&list->lock, flags);
	cyg_spinlock_clear_intsave(&list->lock, flags);
}
...
static __inline void skb_insert(struct sk_buff *old, struct sk_buff *newsk)
{
	...
	cyg_spinlock_spin_intsave(&old->list->lock, &flags);
	__skb_insert(newsk, old->prev, old, old->list);
	//spin_unlock_irqrestore(&old->list->lock, flags);
	cyg_spinlock_clear_intsave(&old->list->lock, flags);
}
...
static __inline unsigned char *skb_put(struct sk_buff *skb, unsigned int len)
{
	...
	skb->len+=len;
	if(skb->tail>skb->end) {
		skb_over_panic(skb, len, (void*)current_text_addr());
	}
	return tmp;
}
...
static __inline unsigned char *skb_push(struct sk_buff *skb, unsigned int len)
{
	...
	if(skb->data<skb->head) {
		skb_under_panic(skb, len, (void*)current_text_addr());
	}
	return skb->data;
}
...
skb_cow(struct sk_buff *skb, unsigned int headroom)
{
	...
	if (delta || skb_cloned(skb))
		return pskb_expand_head(skb, (delta+15)&~15, 0, 0);
	return 0;
}
...
static __inline struct sk_buff *skb_padto(struct sk_buff *skb, unsigned int len)
{
...
extern unsigned int		datagram_poll(struct CYG_FILE_TAG *file, struct socket *sock/*, struct poll_table_struct *wait*/);
...
extern int			skb_copy_and_csum_datagram(const struct sk_buff *skb, int offset, cyg_uint8 *to, int len, unsigned int *csump);
...
extern unsigned int		skb_copy_and_csum_bits(const struct sk_buff *skb, int offset, cyg_uint8 *to, int len, unsigned int csum);
extern void			skb_copy_and_csum_dev(const struct sk_buff *skb, cyg_uint8 *to);
