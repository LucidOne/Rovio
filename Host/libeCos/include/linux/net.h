...
struct socket
{
	...
	void	*fasync_list;	/* Asynchronous wake up list	*/
	struct CYG_FILE_TAG		*file;		/* File back pointer for gc	*/
	struct sock		*sk;

	short			type;
	unsigned char		passcred;
	cyg_selinfo read_sb_sel;	/* process selecting read */
	cyg_selinfo write_sb_sel;	/* process selecting write */
	short read_flag_sel;
	short write_flag_sel;
...
struct proto_ops {
  ...
  unsigned int (*poll)	(struct CYG_FILE_TAG *file, struct socket *sock/*, struct poll_table_struct *wait*/);
  ...
  int	(*mmap)		(struct CYG_FILE_TAG *file, struct socket *sock/*, struct vm_area_struct * vma*/);
...
#if 0//clyu
extern int	sock_readv_writev(int type, struct inode * inode, struct CYG_FILE_TAG * file,
...
SOCKCALL_UWRAP(name, poll, (struct CYG_FILE_TAG *file, struct socket *sock/*, struct poll_table_struct *wait*/), \
	      (file, sock, wait)) \
...
SOCKCALL_WRAP(name, mmap, (struct CYG_FILE_TAG *file, struct socket *sock/*, struct vm_area_struct *vma*/), \
	      (file, sock, vma)) \
	      \
