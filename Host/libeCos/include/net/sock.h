...
static __inline unsigned long __xchg(unsigned long x, volatile void *ptr, int size)
{
	...
	switch (size) {
		case 1: __asm {swpb ret, x, [ptr]}
			break;
		case 4:	__asm {swp ret, x, [ptr]}
			break;

		default: 
			//diag_printf("xchg: bad data size: pc 0x%p, ptr 0x%p, size %d\n",
			diag_printf("xchg: bad data size: ptr 0x%p, size %d\n",
...
#define SOCK_DEBUG(sk, msg...) do { if((sk) && ((sk)->debug)) diag_printf(msg); } while (0)
#else
...
do {	cyg_spinlock_init(&((__sk)->lock.slock), 0); \
	(__sk)->lock.users = 0; \
...
struct sock {
	...
	cyg_uint32			daddr;		/* Foreign IPv4 addr			*/
	cyg_uint32			rcv_saddr;	/* Bound local IPv4 addr		*/
	cyg_uint16			dport;		/* Destination port			*/
	...
	int alarm_flag;	/*alarm enable/disable*/
	...
	union {
		char pad[256];//for hci_pinfo and l2cap_pinfo  space;
	...
	union {
		...
		char pad[256];//for bluez_pinfo space
	} protinfo;  		
...
do {	cyg_spinlock_spin(&((__sk)->lock.slock)); \
	if ((__sk)->lock.users != 0) \
		__lock_sock(__sk); \
	(__sk)->lock.users = 1; \
	cyg_spinlock_clear(&((__sk)->lock.slock)); \
} while(0)
...
#define bh_lock_sock(__sk)	cyg_spinlock_spin(&((__sk)->lock.slock))
#define bh_unlock_sock(__sk)	cyg_spinlock_clear(&((__sk)->lock.slock))
...
{//clyu
	if(waitall && len)
		return len;
	else if((waitall && !len) || (!waitall && !min_t(int, sk->rcvlowat, len)))
		return 1;
	else if(!waitall && min_t(int, sk->rcvlowat, len))
		return min_t(int, sk->rcvlowat, len);
...
#endif
extern cyg_uint32 sysctl_wmem_max;
extern cyg_uint32 sysctl_rmem_max;
