...
__packed struct rfcomm_hdr {
	cyg_uint8 addr;
	cyg_uint8 ctrl;
	cyg_uint8 len;    // Actual size can be 2 bytes
} ;//__attribute__ ((packed));

__packed struct rfcomm_cmd {
	cyg_uint8 addr;
	cyg_uint8 ctrl;
	cyg_uint8 len;
	cyg_uint8 fcs;
...
__packed struct rfcomm_mcc {
	cyg_uint8 type;
	cyg_uint8 len;
} ;//__attribute__ ((packed));

__packed struct rfcomm_pn {
	cyg_uint8  dlci;
	cyg_uint8  flow_ctrl;
	cyg_uint8  priority;
	cyg_uint8  ack_timer;
	cyg_uint16 mtu;
	cyg_uint8  max_retrans;
	cyg_uint8  credits;
} ;//__attribute__ ((packed));

__packed struct rfcomm_rpn {
	cyg_uint8  dlci;
	cyg_uint8  bit_rate;
	cyg_uint8  line_settings;
	cyg_uint8  flow_ctrl;
	cyg_uint8  xon_char;
	cyg_uint8  xoff_char;
	cyg_uint16 param_mask;
} ;//__attribute__ ((packed));

__packed struct rfcomm_rls {
	cyg_uint8  dlci;
	cyg_uint8  status;
} ;//__attribute__ ((packed));

__packed struct rfcomm_msc {
	cyg_uint8  dlci;
	cyg_uint8  v24_sig;
} ;//__attribute__ ((packed));
...
struct rfcomm_dlc {
	...
	cyg_uint32          mtu;
	cyg_uint32          cfc;
	cyg_uint32          rx_credits;
	cyg_uint32          tx_credits;

	void          *owner;

	void (*data_ready)(struct rfcomm_dlc *d, struct sk_buff *skb);
	void (*state_change)(struct rfcomm_dlc *d, int err);
	void (*modem_status)(struct rfcomm_dlc *d, cyg_uint8 v24_sig);
};
...
extern cyg_handle_t rfcomm_thread;
extern unsigned long rfcomm_event;
extern cyg_sem_t rfcomm_worker_sem;
...
//	printf("schedule=%x ",event);
	//wake_up_process(rfcomm_thread);
	//if(rfcomm_thread != cyg_thread_self())
	//	cyg_thread_resume(rfcomm_thread);
	cyg_semaphore_post(&rfcomm_worker_sem);
}

//extern struct semaphore rfcomm_sem;
extern cyg_mutex_t  rfcomm_sem;
#define rfcomm_lock()	cyg_mutex_lock( &rfcomm_sem );//down(&rfcomm_sem);
#define rfcomm_unlock()	cyg_mutex_unlock( &rfcomm_sem );//up(&rfcomm_sem);
...
#define rfcomm_dlc_lock(d)	cyg_spinlock_spin(&d->lock)
#define rfcomm_dlc_unlock(d)	cyg_spinlock_clear(&d->lock)
...
/* ---- RFCOMM chechsum ---- */
extern cyg_uint8 rfcomm_crc_table[];

/* ---- RFCOMM sockets ---- */
struct sockaddr_rc {
	uint8_t     rc_len;		/* total length */
	sa_family_t rc_family;
	bdaddr_t    rc_bdaddr;
	cyg_uint8   rc_channel;
};
...
int  rfcomm_connect_ind(struct rfcomm_session *s, cyg_uint8 channel, struct rfcomm_dlc **d);
...
struct rfcomm_dev_list_req {
	cyg_uint16      dev_num;
	struct   rfcomm_dev_info dev_info[1];//clyu 0-->1
};
