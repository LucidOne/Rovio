...
struct wpa_global {
	...
	cyg_cond_t set_ip_cond;
	cyg_mutex_t set_ip_mutex;
	enum SET_WIFI_STATE_E	set_wifi_status;
	enum SET_IP_STATE_E		set_ip_status;
	int						need_set_ip;
	int						need_set_wlan;
};
...
static inline const u8 * wpa_drv_get_mac_addr(struct wpa_supplicant *wpa_s)
{
...

	{
		...
		else
		{
			...
			{
				printf("Get netmask error\n");
			}
			close(fd);
		}
		memcpy(own_addr, (char *)ifr.ifr_hwaddr.sa_data, sizeof(own_addr));
	}
	return (const u8*)own_addr;
...
static inline int wpa_drv_set_mode(struct wpa_supplicant *wpa_s,
					  int mode)
{
