...
static void usage(void)
{
	...
	diag_printf("  -p = driver parameters\n"
	       "  -P = PID file\n"
...
#if 1
int getopt(int argc, char *const *argv, char const*str)
{
}
char *optarg;
void alarm(int time)
...
int wpa_supplicant_main(int argc, char *argv[])
{
...
#if 1	//xhchen, specify default parameter
	// -B
	//params.daemonize++;
	params.daemonize = 0;
	...
	for (;;) {
		...
		case 'd':
			...
			diag_printf("Debugging disabled with "
	...
	if (global == NULL) {
		diag_printf("Failed to initialize wpa_supplicant\n");
		exitcode = -1;
		goto out;
	}
