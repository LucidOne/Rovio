...
struct wpa_ctrl * wpa_ctrl_open(int fd, int pipe_index)
{
	struct wpa_ctrl *ctrl;
	char buf[128];
	size_t len;
		     ...
		     void (*msg_cb)(char *msg, size_t len))
{
	...
	fix_size_write(ctrl->s, &_cmd_len, sizeof(_cmd_len));
	if (fix_size_write(ctrl->s, _cmd, _cmd_len) < 0) {
		os_free(cmd_buf);
		return -1;
	}
	os_free(cmd_buf);
...
int wpa_ctrl_recv(struct wpa_ctrl *ctrl, char *reply, size_t *reply_len)
{
	...
	fix_size_read(ctrl->s, &len, sizeof(len));
	if (len > *reply_len)
	{
		len = *reply_len;
		diag_printf("fix_size_read error\n");
		*reply_len = 0;
		return 0;
	}
	
	res = fix_size_read(ctrl->s, reply, len);
	if (res < 0)
		return res;
	*reply_len = len;
	return 0;
