...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s, wpa_states state)
{
	...
	if (state == WPA_ASSOCIATED)
		wsp_init_interface("wlan0");
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		wpa_s->global->set_wifi_status = SET_WIFI__OK;
		wpa_s->global->set_ip_status = SET_IP__NO_INIT;
		wsp_set_ip_addr("wlan0", &wpa_s->global->set_ip_status);
		cyg_cond_signal(&wpa_s->global->set_ip_cond);
	...
	{
		wpa_s->global->set_wifi_status = SET_WIFI__OK;
		wsp_connected_callback();
	}
	else if (state != WPA_SCANNING)
	{
		wpa_s->global->set_wifi_status = SET_WIFI__TRYING;
		wsp_connecting_callback();
	}
}
...
static void wpa_supplicant_reconfig_receiver(int fd, void *eloop_ctx,
				    void *fd_ctx)
{
	char c;
	struct wpa_global *global = eloop_ctx;
	...
	fix_size_read(fd, &c, sizeof(c));
	
	/* Check set wlan state */
	cyg_mutex_lock(&g_wpa_global->set_ip_mutex);
	need_set_wlan	= global->need_set_wlan;
	if (need_set_wlan)
	{
		global->need_set_wlan = 0;
		global->set_ip_status = SET_IP__NO_INIT;
		global->set_wifi_status = SET_WIFI__TRYING;
	}
	diag_printf("need_set_wlan = %d\n", need_set_wlan);
	cyg_mutex_unlock(&g_wpa_global->set_ip_mutex);
	if (need_set_wlan)
		wpa_supplicant_reconfig(0, eloop_ctx, NULL);

	/* Check set IP state */
	cyg_mutex_lock(&g_wpa_global->set_ip_mutex);
	need_set_ip		= global->need_set_ip;
	if (need_set_ip)
	{
		global->need_set_ip = 0;
		global->set_ip_status = SET_IP__NO_INIT;
	}
	diag_printf("need_set_ip = %d\n", need_set_ip);
	cyg_mutex_unlock(&g_wpa_global->set_ip_mutex);
	if (need_set_ip)
	{
		for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
				if (wpa_supplicant_get_state(wpa_s) == WPA_COMPLETED) {
					wsp_set_ip_addr("wlan0", &global->set_ip_status);
					cyg_cond_signal(&global->set_ip_cond);
				}
			}
	}
...
static void wpa_supplicant_reconfig_sender(int fd, void *eloop_ctx,
				    char c)
{
	fix_size_write(fd, &c, sizeof(c));
}


static int wpa_supplicant_register_reconfig(struct wpa_global *global)
{
	int pipe_index;
	int fd = pipe_create(&pipe_index);
	if (fd < 0) {
		perror("wpa_supplicant_register_reconfig");
		goto fail;
	}

	eloop_register_read_sock(fd,
				 wpa_supplicant_reconfig_receiver,
				 global, NULL);
...
{
	if (g_wpa_global != NULL)
		wpa_supplicant_reconfig_sender(g_wpa_global->reconfig_fd, g_wpa_global, reconf_msg);
}


void wsp_set_wireless_config()
{
	if (g_wpa_global != NULL)
	{
		cyg_mutex_lock(&g_wpa_global->set_ip_mutex);
		g_wpa_global->need_set_wlan = 1;
		cyg_mutex_unlock(&g_wpa_global->set_ip_mutex);
	
		wsp_reconfig(RECONF_EVENT_RELOAD);
	}
}

void wsp_set_network_ip()
{
	if (g_wpa_global != NULL)
	{
		int need_set = 0;
		cyg_mutex_lock(&g_wpa_global->set_ip_mutex);
		if (g_wpa_global->set_wifi_status == SET_WIFI__OK)
		{
			g_wpa_global->need_set_ip = 1;
		...
		if (need_set)
			wsp_reconfig(RECONF_EVENT_SETIP);
	}
}
...
void wsp_wait_network_ip(int timeout_ticks)
{
	...
	{
		cyg_mutex_lock(&g_wpa_global->set_ip_mutex);
		if (g_wpa_global->need_set_ip
			|| g_wpa_global->set_ip_status == SET_IP__NO_INIT
			|| g_wpa_global->set_ip_status == SET_IP__DHCP_TRYING
			|| g_wpa_global->set_ip_status == SET_IP__STATIC_IP_TRYING)
		{			
			cyg_cond_timed_wait(&g_wpa_global->set_ip_cond, cyg_current_time() + timeout_ticks);
		}
		cyg_mutex_unlock(&g_wpa_global->set_ip_mutex);
	}
}
...
void wsp_get_config_state(enum SET_WIFI_STATE_E *wifi_state, enum SET_IP_STATE_E *ip_state)
{
	...
	{
		if (wifi_state != NULL)
			*wifi_state = g_wpa_global->set_wifi_status;
		if (ip_state != NULL)
			*ip_state = g_wpa_global->set_ip_status;
	}
	else
	{
		if (wifi_state != NULL)
			*wifi_state = SET_WIFI__NO_INIT;
		if (ip_state != NULL)
			*ip_state = SET_IP__NO_INIT;
	}
}
			      ...
			      struct wpa_ssid *ssid)
{
	...
	//xhchen add: set mode
	if (ssid->mode == 0)
		wpa_drv_set_mode(wpa_s, 1);
	if (ssid->mode == 1)
		wpa_drv_set_mode(wpa_s, 0);
...
int wpa_supplicant_run(struct wpa_global *global)
{
	...
	/* To send set ip message */
	cyg_mutex_init(&global->set_ip_mutex);
	cyg_cond_init(&global->set_ip_cond, &global->set_ip_mutex);
	global->set_ip_status	= SET_IP__NO_INIT;
	global->set_wifi_status = SET_WIFI__NO_INIT;
	global->need_set_ip		= 0;
	global->need_set_wlan	= 0;
	
	wpa_supplicant_register_reconfig(global);
