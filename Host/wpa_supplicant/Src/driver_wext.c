...
static int g_kernel_driver_fd = 0;
static int g_kernel_driver_ifindex = 0;
void wpa_kernel_driver_wext_set_param(int fd, int ifindex)
{
	g_kernel_driver_fd = fd;
	g_kernel_driver_ifindex = ifindex;
}
...
int wpa_kernel_driver_wext_send_link_event(const char *buffer, int len)
{
	...
	if (g_kernel_driver_fd == 0)
	{
		diag_printf("Can not send link event\n");
		return -1;
	}
	
	diag_printf("send kernel event=%d,%d, len=%d\n", g_kernel_driver_ifindex, g_kernel_driver_fd, len);
	ret = fix_size_write(g_kernel_driver_fd, buffer, len);
	if (ret < 0) {
...
int wpa_kernel_driver_wext_send_oper_ifla(int linkmode, int operstate)
{
	struct {
		struct nlmsghdr hdr;
	...
	req.ifinfo.ifi_index = g_kernel_driver_ifindex;
	...
	diag_printf("kernel event=%d,%d\n", g_kernel_driver_ifindex, g_kernel_driver_fd);
	ret = fix_size_write(g_kernel_driver_fd, &req, req.hdr.nlmsg_len);
	if (ret < 0) {
		wpa_printf(MSG_DEBUG, "WEXT: Sending operstate IFLA failed: "
			   "%s (assume operstate is not supported)",
			   strerror(errno));
...
#if 1	//xhchen
	diag_printf("drv(%x)->event_sock=%x\n", drv, drv->event_sock);
	ret = fix_size_write(drv->event_sock, &req, req.hdr.nlmsg_len);
#else
 ...
 * Returns: 0 on success, -1 on failure
 */
int wpa_driver_wext_set_region(void *priv)
{
	struct wpa_driver_wext_data *drv = priv;
	struct iwreq iwr;
	int ret = 0;
	...
	*(int *)iwr.u.name = WLANSETREGION;
	*(int *)(iwr.u.name + 4) =  0x40;	/* Set region code to japan */
	if (ioctl(drv->ioctl_sock, WLAN_SETONEINT_GETNONE, &iwr) < 0) {
		perror("ioctl[WLANSETREGION]");
		ret = -1;
	}	
...
try_again:
	fromlen = sizeof(from);
#if 1	//xhchen
	//left = read(sock, buf, sizeof(buf));
	left = fix_size_read(sock, buf, sizeof(struct nlmsghdr));
	h = (struct nlmsghdr *) buf;
	left += fix_size_read(sock, buf + sizeof(struct nlmsghdr), h->nlmsg_len - left);
#else
...
void * wpa_driver_wext_init(void *ctx, const char *ifname)
{
	...
	if (wpa_driver_wext_set_mode(drv, 0) < 0) {
		diag_printf("Could not configure driver to use managed mode\n");
	}

	if (wpa_driver_wext_get_ifflags(drv, &flags) != 0 ||
	    wpa_driver_wext_set_ifflags(drv, flags | IFF_UP) != 0) {
		diag_printf("Could not set interface '%s' UP\n", drv->ifname);
	}

	wpa_driver_wext_get_range(drv);
	
	wpa_driver_wext_set_region(drv);

	drv->ifindex = if_nametoindex(drv->ifname);
	wpa_kernel_driver_wext_set_param(drv->event_sock, drv->ifindex);
...
int wpa_driver_wext_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if (ssid_len > IW_ESSID_MAX_SIZE) {
		wpa_printf(MSG_DEBUG, "%s: too long SSID (%lu)",
			   __func__, (unsigned long) ssid_len);
		return -1;
	}
				     ...
				     size_t max_size)
{
	...
	for (;;) {
		...
		os_memset(&iwr, 0, sizeof(iwr));
		os_strncpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
		iwr.u.data.pointer = (void*)res_buf;
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		if (drv->we_version_compiled >= 0 &&
		    (iwe->cmd == SIOCGIWESSID ||
		     //iwe->cmd == SIOCGIWENCODE ||	/* xhchen: remove this line since GHGuo change the event type in eCos driver. */
		     iwe->cmd == IWEVGENIE ||
		     iwe->cmd == IWEVCUSTOM)) {
			/* WE-19 removed the pointer from struct iw_point */
			char *dpos = (char *) &iwe_buf.u.data.length;
...
diag_printf("iwd: pos=%x, custom=%x, cmd=%x, len=%d\n", (int)pos, (int)custom, (int)iwe->cmd, (int)iwe->len);
		switch (iwe->cmd) {
		case SIOCGIWAP:
diag_printf("SIOCGIWAP\n");		
			if (!first)
				ap_num++;
			first = 0;
			if (ap_num < max_size) {
...
diag_printf("IWEVQUAL\n");		
			...
			break;
		case SIOCGIWENCODE:
diag_printf("SIOCGIWENCODE\n");
			if (ap_num < max_size &&
			    !(iwe->u.data.flags & IW_ENCODE_DISABLED))
				results[ap_num].caps |= IEEE80211_CAP_PRIVACY;
			break;
		case SIOCGIWRATE:
diag_printf("SIOCGIWRATE\n");
