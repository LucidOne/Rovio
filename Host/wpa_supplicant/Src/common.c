...
#endif /* CONFIG_DEBUG_FILE */
	diag_printf("%ld.%06u: ", (long) tv.sec, (unsigned int) tv.usec);
}
...
#endif /* CONFIG_DEBUG_FILE */
	diag_printf("%s - hexdump(len=%lu):", title, (unsigned long) len);
	if (buf == NULL) {
		diag_printf(" [NULL]");
	} else if (show) {
		for (i = 0; i < len; i++)
...
#endif /* CONFIG_DEBUG_FILE */
	if (!show) {
		diag_printf("%s - hexdump_ascii(len=%lu): [REMOVED]\n",
		       title, (unsigned long) len);
		return;
	}
	if (buf == NULL) {
		diag_printf("%s - hexdump_ascii(len=%lu): [NULL]\n",
		       title, (unsigned long) len);
		return;
	}
	diag_printf("%s - hexdump_ascii(len=%lu):\n", title, (unsigned long) len);
	while (len) {
		llen = len > line_len ? line_len : len;
		diag_printf("    ");
		for (i = 0; i < llen; i++)
			diag_printf(" %02x", pos[i]);
		for (i = llen; i < line_len; i++)
			diag_printf("   ");
		diag_printf("   ");
		for (i = 0; i < llen; i++) {
