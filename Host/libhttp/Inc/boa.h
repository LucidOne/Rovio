...
extern HASH_MEM_POOL_HEADER_T *g_HTTP_HASH_MEM_POOL;
#endif

#include "vp_bitrate_control.h"

#ifdef HTTP_FLASH_BUFFER
...
void send_r_request_ok_param(request * req, const char *pcEncodings);	/* 200 */
...
int DecodeBase64(void *dst, int dstlen, char *src,int srclen);
#if 0
...
int toSDPath(const char *src, char *des, int *len);

HTTP_PRIVILEGE_T *getHttpPrivilege(HTTPCONNECTION hConnection);
