...
extern AU_I2S_MCLK_TYPE_E _eI2SMclkType;
...
static void IIS_Set_Sample_Frequency(int choose_sf){
	if (_eI2SMclkType == I2S_MCLK_TYPE_NORMAL)
	{
		switch (choose_sf)
		{
			...
			case AU_SAMPLE_RATE_11025:					//11.025KHz
				_uIISCR = _uIISCR | FS_384 | SCALE_4 | BCLK_32;//APLL=16.934Mhz
				break;
			...
			case AU_SAMPLE_RATE_48000:						//48KHz
				...
				_uIISCR = _uIISCR | FS_384 | SCALE_1 | BCLK_32 ;
				break;
			default:break;
		}
		outpw(REG_ACTL_IISCON,_uIISCR);
		return;
	}else if (_eI2SMclkType == I2S_MCLK_TYPE_256FS)
	{
		switch (choose_sf)
		{
			case AU_SAMPLE_RATE_8000:							//8KHz
		...
		outpw(REG_ACTL_IISCON,_uIISCR);
		return;
	}else if (_eI2SMclkType == I2S_MCLK_TYPE_APLL)
	{
		switch (choose_sf)
		{
			case AU_SAMPLE_RATE_8000:							//8KHz
