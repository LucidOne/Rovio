 ...
 *     
 * HISTORY
 *     2005.11.01		Created by Shih-Jen Lu
 *
 *
 * REMARK
 *     None
...
static BOOL	 _bBYPASS = 0;

INT WM8731_DAC_Setup(void);
INT WM8731_ADC_Setup(void);
...
#endif
static INT WM8731_Set_DAC_Volume(UINT8 ucLeftVol, UINT8 ucRightVol);
static INT WM8731_Set_ADC_Volume(UINT8 ucLeftVol, UINT8 ucRightVol);
...
/*                                                                       */
/*************************************************************************/
INT  wm8731StartPlay(AU_CB_FUNC_T *fnCallBack, INT nSamplingRate, 
...
INT  wm8731StopPlay()
{
	...
	//WM_Write_Data(0x0000);//R0 software reset;all registers reture to default value
...
/*                                                                       */
/*************************************************************************/
INT  wm8731StartRecord(AU_CB_FUNC_T *fnCallBack, INT nSamplingRate, 
							INT nChannels, INT data_format)
{
	INT		nStatus;
...
/*                                                                       */
/*************************************************************************/
INT  wm8731SetPlayVolume(UINT8 ucLeftVol, UINT8 ucRightVol)  //0~31
{
	INT nRetValue = 0;
	if (ucLeftVol>31)
		ucLeftVol=31;
	...
	nRetValue = WM8731_Set_DAC_Volume(ucLeftVol,ucRightVol);
	if (nRetValue!=0){
		return nRetValue;
	}
	return 0;
...
/*                                                                       */
/*************************************************************************/
INT  wm8731SetRecordVolume(UINT8 ucLeftVol, UINT8 ucRightVol)  //0~31
{
	if (ucLeftVol>31)
		ucLeftVol=31;
	if (ucRightVol>31)
	...
	WM8731_Set_ADC_Volume(ucLeftVol,ucRightVol);
...
/*---------- for WM8731	functions group----------*/
//(MIN)  0 ~ (MAX) 255
static INT WM8731_Set_DAC_Volume(UINT8 ucLeftVol, UINT8 ucRightVol){	//0~31
	...
	if (ucLeftVol!=0)
		...
		data = 0;
	WM_Write_Data(2,data);
	
	
	if (ucRightVol!=0)
		data = (ucRightVol*3 + 34);
	else
		data = 0;
	WM_Write_Data(3,data);
	if (_bBYPASS)
	{
		WM_Write_Data(0,ucLeftVol);//bypass vol
		WM_Write_Data(1,ucRightVol);//bypass vol
	}
	return 0;
}	
static INT WM8731_Set_ADC_Volume(UINT8 ucLeftVol, UINT8 ucRightVol){
	...
	for (nTimeOutCount=RETRYCOUNT;nTimeOutCount>0;nTimeOutCount--){	
		nRetValue = WM_Write_Data(0,data);
		if (nRetValue == 0)
			break;
	}
	if (nTimeOutCount == 0){
	...
	if (ucRightVol!=0)
		...
		data = 0;
	for (nTimeOutCount=RETRYCOUNT;nTimeOutCount>0;nTimeOutCount--){	
		nRetValue = WM_Write_Data(1,data);
		if (nRetValue == 0)
			break;
	}
	if (nTimeOutCount == 0){
...
INT WM8731_DAC_Setup(){

		
	WM_Write_Data(4,0x1A);//R4 DACSEL enable bypass enable ADC mute
	WM8731_Set_DAC_Volume(0,0);//r11 r12 set volume
	WM_Write_Data(5,0x00);//R5 disable DAC mute 
	WM_Write_Data(7,0x012);//R7 slave mode,I2S format, 16bit data length
	WM_Write_Data(8,0x0);//R8 mclk=256fs
	WM_Write_Data(9,0x1);//R9 active audio interface	
	WM_Write_Data(6,0x067);//R6 disable POWEROFF, DACPD, OUTPD
	if (_bBYPASS)
	{
		WM_Write_Data(4,0xa);
		WM_Write_Data(6,0x6e);
	}
...
INT WM8731_ADC_Setup(){

	
	
	//WM_Write_Data(0xf,0x00);//software reset
	WM_Write_Data(4,0x005)	;//R4 enable boost, disable ADC mute
	WM_Write_Data(5,0x008);//R5 De-emphasis disabled, DAC mute, ADC HPF enable
	WM_Write_Data(6,0x079);//R6 enable: power, ADC, MIC, 
						   //	disable: Oscillator, LINEOUT, DAC, LINEIN, CLKOUT
	WM_Write_Data(7,0x012);//R7 slave mode,I2S format, 16bit data length	
	WM_Write_Data(8,0x0);//R8 mclk=256fs
	WM_Write_Data(9,0x1);//R9 active audio interface
	...
	{
		WM_Write_Data(4,0xa);
		WM_Write_Data(6,0x6a);
	}
		
	return 0;
	
}   
/*---------- end of WM8731's functions group ----------*/
