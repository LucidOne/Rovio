...
int main()
{
	...
	outpw(REG_UPLLCON, 0x16220);
	outpw(REG_CLKSEL,  0x304);      /* system clock source is from UPLL */  
	...
	//SSum = SSum/NoiseNum;
	//printf("%f\n",SSum);
	THD = (float) (10*log10(MAXNoise) - 10*log10(_1kVal));
	if (THD <= -30)
	{
	...
	outpw(REG_GPIO_DAT, inpw(REG_GPIO_DAT) | (((int)THD & 0xff)>>2) << 14);
