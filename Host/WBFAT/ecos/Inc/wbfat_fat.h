...
typedef	struct sector_buf_S
{
	...
	volatile LDISK_T *ptLDisk; 		/* LDISK_T * associated	with this sector */
	UINT32		uSectorNo;			/* logical sector number */
	volatile BOOL bIsDirty;			/* dirty or not */
	volatile INT  nReaderCnt;		/* for reader task */
#ifdef ECOS
	volatile cyg_handle_t owner;				/* for writer task */
#endif
} 	SEC_BUFF_T;
