...
static TEST_RECV_DATA_T netdata[TEST_RECV_SERVER_NUM];
...
void test_recv_server(cyg_addrword_t pnetdata)
{
	...
	while(1)
	{
		recvlen = recv(new_s, precvbuf, TEST_RECV_MSG_LEN, 0, pbuf, RNT_BUFFER_LEN);
		if(recvlen < 0)
		{
			test_printf_error("test_recv_server");
			ierr = 1;
		...
		if(strcmp(precvbuf, TEST_RECV_MSG) != 0)
		{
			ierr = 1;
			test_printf_error("test_recv_server");
			break;
