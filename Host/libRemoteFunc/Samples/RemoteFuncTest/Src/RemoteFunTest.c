...
int main(void)
{
	...
	test_tcp_socket(g_RemoteNet_Buf, RNT_BUFFER_LEN);
	test_udp_socket(g_RemoteNet_Buf, RNT_BUFFER_LEN);
	test_netread();
	test_netwrite();
	test_send();
	test_recv();
	...
	test_sendto();
	test_netselect();
	test_getsockname(g_RemoteNet_Buf, RNT_BUFFER_LEN);
	...
	//test_netioctl contain ip configuration and will inflect route table and gateway
	//So this test should behind test_netioctl_withbuf and test_wb740getgateway
	test_netioctl(g_RemoteNet_Buf, RNT_BUFFER_LEN);
	test_wb740reboot(g_RemoteNet_Buf, RNT_BUFFER_LEN);
