...
static char msg[TEST_NETREAD_SERVER_NUM][TEST_NETREAD_MSG_LEN];
...
void test_netread_server(cyg_addrword_t pnetdata)
{
	...
	while(1)
	{
		readlen = netread(new_s, preadbuf, TEST_NETREAD_MSG_LEN, pbuf, RNT_BUFFER_LEN);
		if(readlen < 0)
		{
			test_printf_error("test_netread_server");
			ierr = 1;
		...
		if(readlen > 0)
		{
			if(strcmp(preadbuf, TEST_NETREAD_MSG) != 0)
			{
				ierr = 1;
				test_printf_error("test_netread_server");
				break;
