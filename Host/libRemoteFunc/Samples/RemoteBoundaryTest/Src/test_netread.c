...
{
	test_printf_begin("test_netread");
	test_netread_entry();
	test_printf_end("test_netread");
}
...
static char msg[TEST_NETREAD_SERVER_NUM * 2][TEST_NETREAD_MSG_LEN];
static TEST_NETREAD_DATA_T netdata[TEST_NETREAD_SERVER_NUM];

void test_netread_entry(void)
{
	int iPort = TEST_NETREAD_SERVER_BEGIN_PORT;
	int i;
	
	for(i = 0; i < TEST_NETREAD_SERVER_NUM; i++)
	{
		netdata[i].iport = iPort+i;
		netdata[i].pbuf = (char*)NON_CACHE(g_RemoteNet_Buf1[i]);
		netdata[i].preadbuf = msg[i];
		cyg_thread_create(THREAD_PRIORITY, &test_netread_server, (cyg_addrword_t)&netdata[i], 
	...
	for(; i < TEST_NETREAD_SERVER_NUM * 2; i++)
	{
		netdata[i].iport = iPort+i;
		netdata[i].pbuf = (char*)NON_CACHE(g_RemoteNet_Buf1[i]);
		netdata[i].preadbuf = msg[i];
		cyg_thread_create(THREAD_PRIORITY, &test_netread_client, (cyg_addrword_t)&netdata[i], 
						NULL, thread_stack[i], STACK_SIZE, &thread_handle[i], &thread[i]);
		cyg_thread_resume(thread_handle[i]);
	}
	
	for(i = 0; i < TEST_NETREAD_SERVER_NUM * 2; i++)
	{
...
void test_netread_server(cyg_addrword_t pnetdata)
{
	...
	int port = ((TEST_NETREAD_DATA_T*)pnetdata)->iport;
	char *pbuf = ((TEST_NETREAD_DATA_T*)pnetdata)->pbuf;
	char *preadbuf = ((TEST_NETREAD_DATA_T*)pnetdata)->preadbuf;
	
	threadid = cyg_thread_self();
	
	if((hp = gethostbyname(TEST_REMOTEFUNC_HOSTNAME, pbuf, RNT_BUFFER_LEN)) == NULL)
	{
		test_printf_error("test_netread_server");
		cyg_thread_exit();
	}
	...
	if(netread(-1, preadbuf, TEST_NETREAD_MSG_LEN, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	if(netread(new_s, NULL, TEST_NETREAD_MSG_LEN, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	if(netread(new_s, preadbuf, 0, pbuf, RNT_BUFFER_LEN) != 0)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	if(netread(new_s, preadbuf, -1, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	if(netread(new_s, preadbuf, TEST_NETREAD_MSG_LEN, NULL, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	if(netread(new_s, preadbuf, TEST_NETREAD_MSG_LEN, pbuf, 0) != -1)
	{
		test_printf_error("test_netread_server");
		goto fail;
	}
	
	test_printf_success("test_netread_server");
...
void test_netread_client(cyg_addrword_t pnetdata)
{
	int s, readlen;
	struct sockaddr_in sa, r_sa;
	struct hostent *hp;
	...
	if((hp = gethostbyname(TEST_REMOTEFUNC_HOSTNAME, pbuf, RNT_BUFFER_LEN)) == NULL)
	{
		test_printf_error("test_netread_client");
		cyg_thread_exit();
	}
	...
	sa.sin_port = htons(IPPORT_USERRESERVED + port - TEST_NETREAD_SERVER_NUM);
	...
	readlen = netread(s, preadbuf, TEST_NETREAD_MSG_LEN, pbuf, RNT_BUFFER_LEN);
	netclose(s, pbuf, RNT_BUFFER_LEN);
	cyg_thread_exit();
}
