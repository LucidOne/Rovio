...
static TEST_SEND_DATA_T netdata[TEST_NETREAD_SERVER_NUM];
...
void test_send_entry(void)
{
	...
	for(; i < TEST_SEND_SERVER_NUM * 2; i++)
	{
		netdata[i].iport = iPort+i;
		netdata[i].pbuf = (char*)NON_CACHE(g_RemoteNet_Buf1[i]);
		netdata[i].psendbuf = msg[i];
		cyg_thread_create(THREAD_PRIORITY, &test_send_client, (cyg_addrword_t)&netdata[i], 
						NULL, thread_stack[i], STACK_SIZE, &thread_handle[i], &thread[i]);
		cyg_thread_resume(thread_handle[i]);
	}
	
	for(i = 0; i < TEST_SEND_SERVER_NUM * 2; i++)
	{
...
void test_send_server(cyg_addrword_t pnetdata)
{
	...
	if(send(-1, psendbuf, TEST_SEND_MSG_LEN, 0, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
	
	if(send(new_s, NULL, TEST_SEND_MSG_LEN, 0, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
	
	if(send(new_s, psendbuf, 0, 0, pbuf, RNT_BUFFER_LEN) != 0)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
	
	if(send(new_s, psendbuf, -1, 0, pbuf, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
	
	if(send(new_s, psendbuf, TEST_SEND_MSG_LEN, 0, NULL, RNT_BUFFER_LEN) != -1)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
	
	if(send(new_s, psendbuf, TEST_SEND_MSG_LEN, 0, pbuf, 0) != -1)
	{
		test_printf_error("test_send_server");
		goto fail;
	}
...
void test_send_client(cyg_addrword_t pnetdata)
{
	int s, readlen;
	struct sockaddr_in sa, r_sa;
	struct hostent *hp;
	...
	if((hp = gethostbyname(TEST_REMOTEFUNC_HOSTNAME, pbuf, RNT_BUFFER_LEN)) == NULL)
	{
		test_printf_error("test_send_client");
		cyg_thread_exit();
	}
	...
	sa.sin_port = htons(IPPORT_USERRESERVED + port - TEST_SEND_SERVER_NUM);
	...
	readlen = recv(s, psendbuf, TEST_SEND_MSG_LEN, 0, pbuf, RNT_BUFFER_LEN);
	netclose(s, pbuf, RNT_BUFFER_LEN);
	cyg_thread_exit();
}
